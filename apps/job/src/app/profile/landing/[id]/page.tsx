...
import { ArrowLeft, MapPin, Briefcase, GraduationCap, Award, Users, Heart, MessageCircle, Share2 } from 'lucide-react';
...
export default function ProfileLandingPage() {
    ...
    const handleRevealContact = () => {
        ...
        // Navigate to their main profile where they can reveal contact
    ...
    if (loading) {
        return (
            <div style={{ minHeight: '100vh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ width: 40, height: 40, borderRadius: '50%', border: '3px solid #333', borderTop: '3px solid #FF0069', animation: 'spin 0.75s linear infinite' }} />
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#000', color: '#fff' }}>
            ...
            <div style={{
                ...
                <button onClick={() => router.back()} style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', padding: 8 }}>
                    <ArrowLeft size={24} />
                </button>
            ...
            <div style={{ padding: '24px', textAlign: 'center' }}>
                ...
                {/* Name */}
                <h1 style={{ fontSize: 24, fontWeight: 700, fontFamily: 'Poppins', marginBottom: 4 }}>
                ...
                {/* Location */}
                {profile.location && (
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, color: '#888', fontSize: 14 }}>
                        <MapPin size={14} />
            ...
            {profile.bio && (
                <div style={{ padding: '0 24px', marginBottom: 24 }}>
                    ...
                    <p style={{ fontSize: 14, color: '#ccc', lineHeight: 1.6 }}>{profile.bio}</p>
                </div>
            )}
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                        {profile.skills.map((skill: string, i: number) => (
                            <span key={i} style={{
                                ...
                                border: '1px solid #FF0069', borderRadius: 20, fontSize: 13, color: '#fff'
                            }}>
                                {skill}
                            </span>
                        ))}
                ...
                <div style={{ padding: '0 24px', marginBottom: 24 }}>
                    ...
                    {profile.experience.map((exp: any, i: number) => (
                        <div key={i} style={{ marginBottom: 16, paddingLeft: 16, borderLeft: '2px solid #333' }}>
                            <div style={{ fontWeight: 600, fontSize: 14 }}>{exp.title || exp.position}</div>
                            <div style={{ fontSize: 13, color: '#888' }}>{exp.company} • {exp.duration}</div>
                        </div>
                    ))}
                </div>
            )}
                ...
                <div style={{ padding: '0 24px', marginBottom: 24 }}>
                    ...
                    {profile.education.map((edu: any, i: number) => (
                        <div key={i} style={{ marginBottom: 16, paddingLeft: 16, borderLeft: '2px solid #333' }}>
                            ...
                            <div style={{ fontSize: 13, color: '#888' }}>{edu.school} • {edu.year}</div>
                        </div>
                    ))}
                </div>
            )}
