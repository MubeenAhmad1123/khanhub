// src/components/sukoon/client-profile/TherapyTab.tsx
import React, { useState, useEffect, useCallback } from 'react';
import { TherapySession } from '@/types/sukoon';
import { getTherapySessions, addTherapySession } from '@/lib/sukoon/clients';
import { Loader2, Plus, Star, Calendar, FileText } from 'lucide-react';
import { toast } from 'react-hot-toast';
import { formatDateDMY, parseDateDMY } from '@/lib/utils';

export default function TherapyTab({ clientId, session }: { clientId: string, session: any }) {
  ...
  const fetchSessions = useCallback(async () => {
    try {
      setLoading(true);
      const data = await getTherapySessions(clientId);
      setSessions(data);
    } catch (error) {
  ...
  const handleAddSession = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newSession: TherapySession = {
        ...
        date,
        sessionNotes,
        clientMood: clientMood || undefined,
        progressRating,
            ...
            <div key={s.id} className="bg-white border border-gray-100 p-6 rounded-[2rem] shadow-sm hover:shadow-md transition-all">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  <div className="bg-purple-50 text-purple-700 font-black text-lg w-12 h-12 rounded-xl flex items-center justify-center border border-purple-100 shadow-inner">
                    #{s.sessionNumber}
                  </div>
                  <div>
            ...
            <form onSubmit={handleAddSession} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                   ...
                   <select value={progressRating} onChange={e => setProgressRating(Number(e.target.value) as 1|2|3|4)} className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-purple-500 outline-none appearance-none">
              ...
              <div>
                <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1.5">Client Mood (Optional)</label>
                <input value={clientMood} onChange={e => setClientMood(e.target.value)} className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-purple-500 outline-none" placeholder="e.g. Anxious, Calm, Defensive" />
              </div>

              <div>
                <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1.5">Session Notes *</label>
                <textarea required rows={4} value={sessionNotes} onChange={e => setSessionNotes(e.target.value)} className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-purple-500 outline-none resize-none" placeholder="Detailed notes about the therapy session..."></textarea>
              </div>
