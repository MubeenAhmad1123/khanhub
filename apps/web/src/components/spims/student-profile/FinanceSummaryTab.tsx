...
export default function FinanceSummaryTab({
  ...
  const { totalPaid, remaining } = useMemo(() => {
    ...
    return { totalPaid: paid, remaining: Math.max(0, pkg - paid) };
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
          ...
          <div className="text-2xl font-black text-gray-900 flex items-center gap-2">
            <Wallet className="text-[#1D9E75]" size={22} />
            Rs {(Number(student.totalPackage) || 0).toLocaleString()}
          </div>
        </div>
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
          <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Total paid (approved)</div>
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
          <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Remaining balance</div>
          <div className="text-2xl font-black text-amber-700">Rs {remaining.toLocaleString()}</div>
        </div>
      </div>
