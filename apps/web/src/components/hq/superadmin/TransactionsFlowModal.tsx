  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center">
      ...
      >
        ...
        <div className="flex items-center justify-between px-7 pt-7 pb-5 border-b border-gray-100 flex-shrink-0">
          <div className="flex items-center gap-3">
            {level === 'txList' && (
              <button
                onClick={goBack}
                className="w-9 h-9 rounded-xl bg-gray-100 flex items-center justify-center hover:bg-gray-200 transition-all active:scale-95 cursor-pointer"
              >
                <ChevronLeft className="w-4 h-4 text-black" />
              </button>
            )}
            <div>
              <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-gray-400 mb-0.5 animate-pulse">
          ...
          <button
            onClick={onClose}
            className="w-10 h-10 rounded-2xl bg-gray-100 flex items-center justify-center hover:bg-gray-200 transition-all active:scale-95 cursor-pointer"
          >
              ...
              >
                ...
                {data && (
                  <motion.button
                    ...
                    className="w-full flex items-center justify-between p-5 rounded-3xl border border-indigo-100 bg-indigo-50/50 hover:bg-indigo-50 transition-all text-indigo-900 font-bold shadow-sm shadow-indigo-600/5 group cursor-pointer"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-[9.5px] text-indigo-500 font-bold uppercase tracking-widest mt-0.5">
                          Combine and print all departments
                        </p>
                      </div>
                    </div>
                    <ArrowUpRight className="w-5 h-5 text-indigo-600 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
                  </motion.button>
                )}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 rounded-2xl border border-emerald-100 bg-emerald-50/30 flex items-center justify-between">
                      <div>
                        ...
                        <span className="text-sm font-black text-emerald-700">{fmtPKR(data.grandIncome)}</span>
                      </div>
                    ...
                    <div className="p-4 rounded-2xl border border-rose-100 bg-rose-50/30 flex items-center justify-between">
                      <div>
                        ...
                        <span className="text-sm font-black text-rose-700">{fmtPKR(data.grandExpense)}</span>
                      </div>
                  ...
                  {data?.departments.map((dept, i) => {
                    ...
                    return (
                      <motion.button
                        ...
                        className={cn(
                          'w-full flex items-center justify-between p-4.5 rounded-2xl border transition-all text-left',
                          hasTransactions
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-xs font-black uppercase tracking-wider text-gray-900 leading-tight">
                              {dept.deptName}
                            </p>
                  ...
                  {activeDept?.transactions.map((tx, i) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="flex items-center justify-between p-4 rounded-2xl border border-zinc-100 bg-white hover:bg-zinc-50 transition-all group"
                      >
                        ...
                        <div className="text-right flex-shrink-0 ml-3">
                          <p className={cn("text-sm font-black tabular-nums", isExp ? "text-rose-600" : "text-emerald-600")}>
                            {isExp ? '-' : '+'}{fmtPKR(tx.amount)}
                          </p>
                          <span className={cn(
                            "text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-full border mt-1 inline-block",
                            tx.status === 'approved'
        ...
        <div className="px-7 py-4 border-t border-gray-100 flex-shrink-0 bg-gray-50/50 flex flex-col gap-3">
