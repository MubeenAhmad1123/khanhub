...
import type { Child } from '@/types/welfare';
      ...
      <div className="flex-1">
        ...
        <div className="flex gap-4 text-xs font-medium uppercase tracking-wider">
          <div className="flex flex-col">
            ...
            <span className="text-gray-700">{formatDateDMY(child.admissionDate)}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-gray-400">Package</span>
            <span className="text-[#1D9E75]">{child.packageAmount.toLocaleString()} PKR</span>
          </div>
        </div>
      </div>
      <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${child.isActive ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
