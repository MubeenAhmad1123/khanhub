// src/components/welfare/child-profile/TherapyTab.tsx
import React, { useState, useEffect, useCallback } from 'react';
import { TherapySession } from '@/types/welfare';
import { getTherapySessions, addTherapySession } from '@/lib/welfare/children';
import { Loader2, Plus, Star, Calendar, User, FileText } from 'lucide-react';
import { toast } from 'react-hot-toast';

export default function TherapyTab({ childId, session }: { childId: string, session: any }) {
  ...
  const fetchSessions = useCallback(async () => {
    try {
      setLoading(true);
      const data = await getTherapySessions(childId);
      setSessions(data);
    } catch (error) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleAddSession} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1.5">Child Mood (Optional)</label>
                <input value={childMood} onChange={e => setChildMood(e.target.value)} className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-teal-500 outline-none" placeholder="e.g. Anxious, Calm, Defensive" />
              </div>
