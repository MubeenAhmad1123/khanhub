...
export async function recalculateGrowthPoints(
  ...
  const dutyDocs = dutySnap.docs.filter(d => d.data().date.startsWith(month));
  ...
  const dressQuery = query(
    collection(db, 'rehab_dress_logs'),
    where('staffId', '==', staffId)
  );
  ...
  const existingDoc = await getDocs(query(collection(db, 'rehab_growth_points'), where('id', '==', `${staffId}_${month}`)));
  const extra = !existingDoc.empty ? (existingDoc.docs[0].data().extra || 0) : 0;

  // 6. Calculate Possible Points
  const [year, monthNum] = month.split('-').map(Number);
  const daysInMonth = new Date(year, monthNum, 0).getDate();
  const totalPossible = daysInMonth * 5; // 5 categories potentially (or custom business logic)
  ...
  await setDoc(doc(db, 'rehab_growth_points', growthPoints.id), growthPoints, { merge: true });
...
export async function getGrowthPoints(
  ...
  const q = query(
    collection(db, 'rehab_growth_points'),
    where('staffId', '==', staffId),
    where('month', '==', month)
  );
