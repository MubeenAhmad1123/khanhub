...
import type { Patient, FeeRecord, CanteenRecord } from '@/types/spims';

export async function getPatient(id: string): Promise<Patient | null> {
  const snap = await getDoc(doc(db, 'spims_patients', id));
...
export async function getPatients(): Promise<Patient[]> {
  const q = query(collection(db, 'spims_patients'), where('isActive', '==', true));
...
export async function createPatient(data: Omit<Patient, 'id' | 'createdAt'>): Promise<string> {
  const res = await addDoc(collection(db, 'spims_patients'), {
    ...data,
...
export async function updatePatient(id: string, data: Partial<Patient>): Promise<void> {
  ...
  await updateDoc(doc(db, 'spims_patients', id), updateData);
}
