...
import type { AttendanceRecord } from '@/types/sukoon';
...
export async function markAttendance(staffId: string, date: string): Promise<string> {
  const res = await addDoc(collection(db, 'sukoon_attendance'), {
    staffId,
    date,
    status: 'present',
...
export async function overrideAttendance(
  ...
  if (snap.empty) {
    ...
    await updateDoc(doc(db, 'sukoon_attendance', snap.docs[0].id), {
      status,
      overriddenBy: adminId
    });
  }
...
export async function getMonthlyAttendance(staffId: string, month: string): Promise<AttendanceRecord[]> {
  const cacheKey = `sukoon_attendance_${staffId}_${month}`;
