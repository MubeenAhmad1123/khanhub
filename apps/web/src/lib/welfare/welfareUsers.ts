...
import { buildEmail } from './welfareAuth';
import type { WelfareUser, WelfareRole } from '@/types/welfare';

export async function createWelfareUser(
  customId: string, 
  password: string, 
  role: WelfareRole, 
  displayName: string, 
  ...
  const userData: Omit<WelfareUser, 'uid'> = {
    customId,
    name: displayName,
    role,
    displayName,
  ...
  await setDoc(doc(db, 'welfare_users', cred.user.uid), {
    ...userData,
...
export async function deactivateUser(uid: string): Promise<void> {
  await updateDoc(doc(db, 'welfare_users', uid), {
    isActive: false
  });
}

export async function getWelfareUserByCustomId(customId: string): Promise<WelfareUser | null> {
  const q = query(collection(db, 'welfare_users'), where('customId', '==', customId));
  const snap = await getDocs(q);
  if (snap.empty) return null;
  const doc = snap.docs[0];
  return { uid: doc.id, ...doc.data() } as WelfareUser;
}
