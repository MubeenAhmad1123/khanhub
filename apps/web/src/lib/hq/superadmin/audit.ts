...
function normalizeAudit(source: 'hq' | 'rehab' | 'spims', id: string, data: any): UnifiedAuditEntry {
  ...
  const actionRaw = String(data.action || data.type || data.event || '').toLowerCase();
  const action =
  ...
  const actorName = String(data.actorName || data.userName || data.byName || data.createdByName || data.name || '—');
  const message = String(data.message || data.title || data.details || data.summary || data.text || data.action || 'Activity');
  ...
  return {
    ...
    action,
    message,
    entityLabel: data.entityLabel || data.entityName || data.patientName || data.studentName || data.staffName,
