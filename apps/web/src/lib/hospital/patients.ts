...
export async function getPatients(): Promise<Patient[]> {
  const cacheKey = 'hospital_patients_list';
  const cached = getCached<Patient[]>(cacheKey);
  if (cached) return cached;

  const q = query(collection(db, 'hospital_patients'), orderBy('serialNumber', 'desc'), limit(100));
...
export async function getAllPatientsWithFinanceSummary(): Promise<PatientFinanceSummary[]> {
  const cacheKey = 'hospital_patients_finance_summary';
  ...
  const [allFees, allCanteen] = await Promise.all([
    fetchInChunks('hospital_fees', patientIds),
    fetchInChunks('hospital_canteen', patientIds)
  ]);
