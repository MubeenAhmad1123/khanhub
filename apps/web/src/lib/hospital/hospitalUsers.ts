...
import { buildEmail } from './hospitalAuth';
import type { HospitalUser, HospitalRole } from '@/types/hospital';
...
export async function createHospitalUser(
  ...
  await setDoc(doc(db, 'hospital_users', cred.user.uid), {
    ...userData,
...
export async function deactivateUser(uid: string): Promise<void> {
  await updateDoc(doc(db, 'hospital_users', uid), {
    isActive: false
  });
}

export async function getHospitalUserByCustomId(customId: string): Promise<HospitalUser | null> {
  const q = query(collection(db, 'hospital_users'), where('customId', '==', customId));
  const snap = await getDocs(q);
  if (snap.empty) return null;
  const doc = snap.docs[0];
  return { uid: doc.id, ...doc.data() } as HospitalUser;
}
