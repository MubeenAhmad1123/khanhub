// src/app/departments/welfare/dashboard/admin/donor-tree/page.tsx
'use client';
...
import { 
  GitFork, Search, Heart, Loader2, ArrowRight, User, 
  ChevronDown, ChevronRight, Award, DollarSign, Users, ArrowLeft, X
} from 'lucide-react';
import { toast } from 'react-hot-toast';
import { LogoLoader } from '@/components/ui';

export default function DonorTreePage() {
  ...
  const globalAnalytics = useMemo(() => {
    ...
    donors.forEach(d => {
      ...
      if (stats) {
        ...
        // Top referrer calculation based on direct referral count
    ...
    // Total income from all transactions that came via referrals
    transactions.forEach(tx => {
      if (tx.donorId) {
  ...
  useEffect(() => {
    ...
    // Helper to traverse up and expand parents
    ...
    <div className="space-y-6 max-w-6xl mx-auto p-4 md:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1">Track advocate networks and referral-driven donation stats.</p>
        </div>
        <Link 
