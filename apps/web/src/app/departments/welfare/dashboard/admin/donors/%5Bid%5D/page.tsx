...
import { 
  ...
  Edit3, Save, Trash2, X, UserCheck, UserCog
} from 'lucide-react';
...
export default function DonorProfilePage({ params }: { params: { id: string } }) {
  ...
  const fetchDonorDetails = async () => {
    try {
      ...
      setSelectedReferrerId(data.referringMemberId || '');
  ...
  const fetchChildrenAndDonors = async () => {
    try {
      ...
      // Fetch active donors list for referral dropdown selection (client-side sort to avoid composite index requirement)
      const qDonors = query(collection(db, 'welfare_donors'), where('isActive', '==', true));
      const snapDonors = await getDocs(qDonors);
      const list = snapDonors.docs
        .map(d => ({ id: d.id, ...d.data() } as Donor))
        .filter(d => d.id !== params.id); // exclude self
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          {!isEditing ? (
            <button 
              onClick={() => {
                setEditForm({ ...donor });
                setSelectedReferrerId(donor.referringMemberId || '');
                setIsEditing(true);
              }} 
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {(isEditing || donor.referringMemberName) && (
            <SectionCard title="4. Referring Member Details" icon={UserCheck}>
              {isEditing && (
                <div className="space-y-1 col-span-full bg-teal-50/20 p-4 border border-teal-150 rounded-xl mb-4 text-left">
                  <label className="block text-[10px] text-teal-800 font-black uppercase tracking-widest leading-tight">Link to Existing Donor Profile</label>
                  <select 
                    ...
                    onChange={(e) => {
                      ...
                      if (val === '') {
                        handleFieldChange('referringMemberId', '');
                        handleFieldChange('referringMemberName', '');
                        handleFieldChange('referringMemberFatherName', '');
                        handleFieldChange('referringMemberMobile', '');
                        handleFieldChange('referringMemberAddress', '');
                      } else {
                        const selectedDnr = donorsList.find(d => d.id === val);
                        if (selectedDnr) {
                          handleFieldChange('referringMemberId', selectedDnr.id);
                          handleFieldChange('referringMemberName', selectedDnr.fullName);
                          handleFieldChange('referringMemberFatherName', selectedDnr.fatherName || '');
                          handleFieldChange('referringMemberMobile', selectedDnr.contactNumber || '');
                          handleFieldChange('referringMemberAddress', selectedDnr.address || '');
                        }
                      }
                    }}
                    className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none h-[42px] mt-1"
                  >
                  ...
                  <p className="text-[10px] text-gray-400 font-medium mt-1">Select from previously added donor profiles to link them automatically, or choose manual typing.</p>
                </div>
              )}
