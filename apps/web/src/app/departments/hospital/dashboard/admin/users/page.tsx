...
export default function UserManagementPage() {
  ...
  const getRoleBadge = (role: string) => {
    switch(role) {
      ...
      case 'staff': return <span className="flex items-center gap-1 bg-emerald-50 text-emerald-700 font-medium px-2.5 py-1 rounded text-xs tracking-wider uppercase border border-emerald-200"><UserCog className="w-3 h-3"/> Staff</span>;
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button 
            ...
            className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-medium shadow-sm transition-colors flex items-center justify-center gap-2"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-center gap-3">
            ...
            <div>
              <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Family</div>
              <div className="text-xl font-black text-gray-900">{familyCount}</div>
            </div>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center flex-shrink-0 font-black tracking-tighter shadow-sm"><UserCog className="w-5 h-5"/></div>
            <div>
              <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Staff</div>
              <div className="text-xl font-black text-gray-900">{staffCount}</div>
            </div>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-center gap-3 sm:col-span-2 lg:col-span-1">
            ...
            <div>
              <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Admins</div>
              <div className="text-xl font-black text-gray-900">{adminCount}</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="flex bg-gray-100 p-1 rounded-lg w-full md:w-auto overflow-x-auto hide-scrollbar">
            <button
              ...
              className={`px-4 py-2 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${activeTab === 'family' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
            >Family</button>
            <button
              onClick={() => setActiveTab('staff')}
              className={`px-4 py-2 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${activeTab === 'staff' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
            >Staff</button>
            <button
              onClick={() => setActiveTab('all')}
              className={`px-4 py-2 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${activeTab === 'all' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
            >All Users</button>
          </div>
          ...
          <div className="relative w-full md:w-96">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all"
            />
          </div>
        </div>
              ...
              {filteredUsers.map(user => (
                <div key={user.id} className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-gray-50/50 transition-colors">
                  <div className="flex items-center gap-3 sm:gap-4 font-black tracking-tight">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center font-black text-lg flex-shrink-0 border border-emerald-100 shadow-sm">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-black text-gray-900 text-sm sm:text-base flex flex-wrap items-center gap-2 truncate uppercase">
                        {user.displayName}
                        {user.isActive ? (
                          <span className="flex items-center gap-1 text-green-600 bg-green-50 px-1.5 py-0.5 rounded-full text-[8px] sm:text-[9px] uppercase font-black tracking-widest border border-green-100 shadow-sm">
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-4 min-w-[150px] border-t sm:border-t-0 pt-3 sm:pt-0 mt-1 sm:mt-0">
                    ...
                    <div className="text-[9px] font-black text-gray-300 uppercase tracking-widest whitespace-nowrap">
                      {user.createdAt?.toDate?.() ? formatDateDMY(user.createdAt.toDate()) : (user.createdAt ? formatDateDMY(new Date(user.createdAt)) : 'N/A')}
                    </div>
                  </div>
                </div>
              ))}
              ...
              <form onSubmit={handleCreateFamilyUser} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500 bg-gray-50 focus:bg-white"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500 bg-gray-50 focus:bg-white font-mono uppercase"
                    placeholder="e.g. HOS-FAM-001"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full border border-gray-200 rounded-xl pl-4 pr-10 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500 bg-gray-50 focus:bg-white"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500 bg-gray-50 focus:bg-white font-mono"
                ...
                <div className="pt-4 flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium py-2.5 rounded-xl transition-colors shadow-sm disabled:opacity-70 flex items-center justify-center gap-2"
                  >
