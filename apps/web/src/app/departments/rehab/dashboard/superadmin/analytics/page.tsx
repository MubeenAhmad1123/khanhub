...
import {
  ...
  Clock, DollarSign, Filter, RefreshCw, BarChart2
} from 'lucide-react';
import {
...
export default function RehabAnalyticsPage() {
    ...
    try {
      ...
      const list = snap.docs.map(doc => {
        ...
        return {
          ...
          substanceOfAddiction: d.substanceOfAddiction || 'Not Specified',
          monthlyPackage: Number(d.monthlyPackage || d.packageAmount || 0),
          overallRemaining: Number(d.remaining || d.amountRemaining || 0)
        };
      });
      setPatients(list);
    } catch (err) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center space-y-4">
          ...
          <p className="text-sm font-semibold text-gray-500">Loading Rehab Analytics...</p>
        </div>
      </div>
    );
  }
  ...
  const currentlyActive = patients.filter(p => p.isActive).length;
  const totalDischarged = patients.filter(p => !p.isActive).length;

  // Average stay calculation (for discharged patients)
  ...
  const totalOutstandingDues = patients.reduce((sum, p) => sum + p.overallRemaining, 0);

  // --- 2. CUMULATIVE ACTIVE PATIENT TREND (BY MONTH OVER PAST 12 MONTHS) ---
  const getTimelineData = () => {
    ...
    // Start with active count 12 months ago
    let runningActive = patients.filter(p => {
      if (!p.admissionDate) return false;
      const twelveMonthsAgo = new Date(now.getFullYear(), now.getMonth() - 11, 1);
      const admittedBefore = p.admissionDate < twelveMonthsAgo;
      const dischargedAfter = !p.dischargeDate || p.dischargeDate >= twelveMonthsAgo;
      return admittedBefore && dischargedAfter;
    }).length;
  ...
  // --- 3. SUBSTANCE OF ADDICTION DISTRIBUTION ---
  const getSubstanceData = () => {
    const counts: Record<string, number> = {};
    patients.forEach(p => {
      const sub = p.substanceOfAddiction || 'Not Specified';
      counts[sub] = (counts[sub] || 0) + 1;
    });
    return Object.entries(counts)
      .map(([name, value]) => ({ name, value }))
      .sort((a, b) => b.value - a.value)
      .slice(0, 8); // Top 8 substances
  };
  ...
  const getMonthComparisonData = () => {
    ...
    for (let day = 1; day <= maxDays; day++) {
      ...
      patients.forEach(p => {
        if (p.admissionDate && p.admissionDate.getFullYear() === compareYear) {
          if (p.admissionDate.getMonth() === monthA && p.admissionDate.getDate() === day) {
            countA++;
          }
          if (p.admissionDate.getMonth() === monthB && p.admissionDate.getDate() === day) {
            countB++;
          }
        }
      });
    ...
    <div className="space-y-8 p-1 sm:p-2">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-gray-900 flex items-center gap-2 tracking-tight uppercase">
            <Activity className="w-8 h-8 text-rose-500 animate-pulse" /> Super Admin Analytics
          </h1>
          <p className="text-sm font-bold text-gray-500 mt-1 uppercase tracking-wider">Detailed patient insights & trend analysis for Rehab Center</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
        <div className="bg-gradient-to-br from-rose-50 to-pink-50 border border-rose-100 p-6 rounded-2xl shadow-sm text-center relative overflow-hidden group hover:shadow-md transition-shadow">
          <div className="absolute right-2 -top-2 opacity-10 text-rose-500 text-7xl font-bold select-none group-hover:scale-110 transition-transform">#</div>
          <Users className="w-8 h-8 text-rose-500 mx-auto mb-2" />
          <div className="text-3xl font-black text-rose-900 tracking-tight">{totalAdmitted}</div>
          <div className="text-xs font-bold text-rose-600 uppercase tracking-widest mt-1">Total Admitted</div>
        </div>
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 p-6 rounded-2xl shadow-sm text-center relative overflow-hidden group hover:shadow-md transition-shadow">
          ...
          <div className="text-xs font-bold text-blue-600 uppercase tracking-widest mt-1">Total Discharged</div>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-fuchsia-50 border border-purple-100 p-6 rounded-2xl shadow-sm text-center relative overflow-hidden group hover:shadow-md transition-shadow">
          ...
          <div className="text-xs font-bold text-purple-600 uppercase tracking-widest mt-1">Avg. Stay Duration</div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm lg:col-span-2 space-y-4">
          <div>
            <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2 uppercase tracking-wide"><TrendingUp className="w-5 h-5 text-rose-500" /> Admissions, Discharges & Active Patients</h2>
            <p className="text-xs text-gray-400">Running stats over the last 12 months</p>
          </div>
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={timelineData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="activeCount" stroke="#8884d8" fillOpacity={1} fill="url(#colorActive)" name="Active Census" strokeWidth={3} />
                <Area type="monotone" dataKey="admissions" stroke="#82ca9d" fillOpacity={1} fill="url(#colorAdmissions)" name="Admissions" strokeWidth={2} />
                <Area type="monotone" dataKey="discharges" stroke="#f43f5e" fillOpacity={0} name="Discharges" strokeWidth={2} strokeDasharray="4 4" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
          <div>
            <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2 uppercase tracking-wide"><BarChart2 className="w-5 h-5 text-rose-500" /> Substance of Addiction</h2>
            <p className="text-xs text-gray-400">Distribution among admitted clients</p>
          </div>
      ...
      <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2 uppercase tracking-wide"><Filter className="w-5 h-5 text-rose-500" /> Month-over-Month Admissions Comparison</h2>
            <p className="text-xs text-gray-400">Compare daily admissions side-by-side between two months</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <select
              ...
              className="bg-gray-50 border border-gray-200 rounded-xl px-3 py-1.5 text-xs outline-none focus:ring-2 focus:ring-rose-500 font-bold text-black"
            >
