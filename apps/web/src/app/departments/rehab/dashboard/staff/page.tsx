...
export default function StaffSelfPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const staffRef = doc(db, 'rehab_users', user.uid);
      let staffSnap = await getDoc(staffRef);
      
      // Fallback to rehab_staff if rehab_users doesn't exist
      if (!staffSnap.exists()) {
        const fallbackSnap = await getDocs(query(collection(db, 'rehab_staff'), where('loginUserId', '==', user.uid)));
      ...
      const tasksSnap = await getDocs(
        query(
          collection(db, 'rehab_special_tasks'), 
          where('staffId', '==', staffId),
          ...
          <div className={`p-8 border-4 border-black ${glassStyle}`}>
            ...
            <div className="space-y-3">
              {staffProfile.duties.map((duty: any, idx: number) => {
                const label = duty.label || duty.dutyName || duty.description || (typeof duty === 'string' ? duty : 'Unknown Duty');
                ...
                return (
                  <div key={idx} className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${isDone ? 'bg-emerald-50 border-emerald-100' : 'bg-white/50 border-white'}`}>
                    <div className="flex items-center gap-4">
                      <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-black ${isDone ? 'bg-emerald-100 text-emerald-600' : 'bg-indigo-50 text-indigo-600'}`}>
                        {idx + 1}
                      </div>
