...
import { 
  ...
  AlertCircle, ChevronRight, Download, Info, Terminal, Sparkles, FileText, CreditCard
} from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const fetchMetrics = useCallback(async (sId: string) => {
    try {
      const prefix = 'it';
      const rawId = sId.startsWith('it_') ? sId.replace('it_', '') : sId;
      const prefixedId = sId.startsWith('it_') ? sId : `it_${sId}`;
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        setLoading(true);
        const userRef = doc(db, 'it_users', parsed.uid);
      ...
      <div className="sticky top-0 z-50 px-4 py-6 md:px-12 bg-[#FCFBF8]/80 backdrop-blur-md border-b border-white">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-2xl ${glassStyle} text-slate-900`}>
              <Terminal size={28} />
            </div>
            <div>
              <h1 className="text-2xl font-black tracking-tight">IT Portal</h1>
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">Infrastructure Profile • Live</p>
            </div>
          </div>
          ...
          <div className={`p-8 rounded-[3rem] ${glassStyle} flex flex-col items-center relative overflow-hidden`}>
            ...
            <div className="relative group">
              ...
              <button 
                ...
                className="absolute -bottom-2 -right-2 p-3 rounded-2xl bg-slate-900 text-white shadow-xl hover:scale-110 transition-all active:scale-95"
              >
                <Camera size={18} />
              </button>
              <input 
            ...
            <p className="text-slate-600 font-black text-[10px] uppercase tracking-widest mt-2">{profile?.designation || 'IT Engineer'}</p>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className={`p-8 md:p-10 rounded-[3.5rem] min-h-[500px] relative overflow-hidden ${glassStyle}`}>
            ...
            {activeTab === 'special_tasks' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center gap-3 mb-8">
                  <div className="p-2 bg-slate-50 rounded-xl text-slate-900"><Target size={20} /></div>
                  <h3 className="text-xl font-black">IT Special Assignments</h3>
                </div>
                <div className="space-y-4">
                  {specialTasks.length > 0 ? specialTasks.map(task => (
                    <div key={task.id} className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#FCFBF8] flex items-center justify-between group transition-transform hover:scale-[1.01]`}>
                      ...
                      <div className="px-3 py-1 rounded-xl bg-white border border-slate-100 text-[10px] font-black text-slate-900 shadow-sm">+{task.points || 0} PTS</div>
                    </div>
                  )) : (
            ...
            {activeTab === 'attendance' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center gap-3 mb-8">
                  <div className="p-2 bg-slate-50 rounded-xl text-slate-900"><Calendar size={20} /></div>
            ...
            {activeTab === 'duty' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center gap-3 mb-8">
                  <div className="p-2 bg-slate-50 rounded-xl text-slate-900"><Activity size={20} /></div>
                ...
                <div className="space-y-4">
                  {duties.length > 0 ? duties.map(duty => (
                    <div key={duty.id} className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#FCFBF8] flex items-center justify-between`}>
                      ...
                      <div className="px-3 py-1 rounded-xl bg-white border border-slate-100 text-[10px] font-black text-slate-900">+{duty.points || 0} PTS</div>
                    </div>
                  )) : (
            ...
            {activeTab === 'dress' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                 <div className="flex items-center gap-3 mb-8">
                  <div className="p-2 bg-slate-50 rounded-xl text-slate-900"><Shirt size={20} /></div>
                  <h3 className="text-xl font-black">Professional Attire Compliance</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {dressLogs.length > 0 ? dressLogs.map(log => (
                    <div key={log.id} className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#FCFBF8] flex items-center justify-between`}>
                      <div className="flex items-center gap-4">
                        <div className={`p-3 rounded-2xl ${log.status === 'yes' ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-300'}`}>
                          <Shirt size={20} />
                        </div>
                        <div>
                      ...
                      {log.status === 'yes' && <div className="px-3 py-1 rounded-xl bg-white border border-slate-100 text-[10px] font-black text-slate-900">+{log.points || 0} PTS</div>}
                    </div>
                  )) : (
                    <div className="col-span-2 py-16 text-center border border-dashed border-gray-100 rounded-xl opacity-40">
            ...
            {activeTab === 'score' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center gap-3 mb-8">
                  <div className="p-2 bg-slate-50 rounded-xl text-slate-900"><TrendingUp size={20} /></div>
            ...
            {activeTab === 'finance' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                  <div className={`p-8 rounded-[3rem] ${neumorphicOutset} bg-slate-900 text-white relative overflow-hidden`}>
            ...
            {activeTab === 'profile' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center gap-3 mb-8">
                  <div className="p-2 bg-slate-50 rounded-xl text-slate-900"><User size={20} /></div>
                  <h3 className="text-xl font-black">Infrastructure Credentials</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div className="space-y-6">
                      ...
                      <div>
                         ...
                         <div className={`p-5 rounded-[2rem] ${neumorphicInset} bg-white flex items-start gap-4`}>
                            <MapPin className="text-slate-900 mt-1" size={18} />
                   ...
                   <div className="space-y-6">
                      ...
                      <div>
                         <p className="text-[9px] font-black uppercase text-slate-400 mb-2 px-2 tracking-widest">Joining Date</p>
                         <div className={`p-5 rounded-[2rem] ${neumorphicInset} bg-white flex items-center gap-4`}>
                            <Calendar className="text-slate-900" size={18} />
                            <p className="text-sm font-bold text-slate-700">{profile?.joiningDate ? formatDateDMY(profile.joiningDate) : 'Not recorded'}</p>
                         </div>
                      </div>
                   </div>
                </div>
