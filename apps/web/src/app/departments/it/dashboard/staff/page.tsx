...
import { useItSession } from '@/hooks/it/useItSession';
import {
...
export default function StaffSelfPage() {
  ...
  const { session: user, loading: sessionLoading } = useItSession();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const staffRef = doc(db, 'it_users', user.uid);
      let staffSnap = await getDoc(staffRef);
      
      if (!staffSnap.exists()) {
        const fallbackSnap = await getDocs(query(collection(db, 'it_staff'), where('loginUserId', '==', user.uid)));
      ...
      const contribSnap = await getDocs(
        query(
          collection(db, 'it_contributions'), 
          where('staffId', '==', staffId),
      ...
      const tasksSnap = await getDocs(
        query(
          collection(db, 'it_special_tasks'), 
          where('staffId', '==', staffId),
  ...
  const handleCheckIn = async () => {
    ...
    try {
      if (!todayRecord) {
        ...
        await addDoc(collection(db, 'it_attendance'), {
        ...
        if (isLate) {
          ...
          await addDoc(collection(db, 'it_fines'), {
            staffId: staffProfile.id,
            amount: 200,
        ...
        await updateDoc(doc(db, 'it_attendance', todayRecord.id), {
  ...
  const handleContribution = async () => {
    ...
    try {
      await addDoc(collection(db, 'it_contributions'), {
        staffId: staffProfile.id,
  ...
  const handleTaskUpdate = async (taskId: string, newStatus: string) => {
    try {
      await updateDoc(doc(db, 'it_special_tasks', taskId), {
        status: newStatus,
