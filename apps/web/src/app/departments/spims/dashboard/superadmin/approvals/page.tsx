...
export default function ApprovalsPage() {
    ...
    try {
      ...
      if (tx && tx.patientId) {
        try {
          ...
          if (tx.category === 'admission_fee') {
            await updateDoc(doc(db, 'spims_students', tx.patientId), {
              admissionStatus: 'paid',
              updatedAt: serverTimestamp(),
            });
          }

          if (tx.category === 'course_installment') {
            const studentSnap = await getDoc(doc(db, 'spims_students', tx.patientId));
            if (studentSnap.exists()) {
              const currentPaid = studentSnap.data().paidCourseFee || 0;
              await updateDoc(doc(db, 'spims_students', tx.patientId), {
                paidCourseFee: currentPaid + tx.amount,
                updatedAt: serverTimestamp(),
              });
            }
          }

          if (tx.category === 'board_registration') {
            await updateDoc(doc(db, 'spims_students', tx.patientId), {
              boardRegistrationStatus: 'paid',
              updatedAt: serverTimestamp(),
            });
          }

          if (tx.category === 'board_examination') {
            await updateDoc(doc(db, 'spims_students', tx.patientId), {
              boardExamFeeStatus: 'paid',
              updatedAt: serverTimestamp(),
            });
          }
          ...
          if (tx.category === 'staff_salary' && tx.staffId) {
            ...
            const salaryQ = query(
              collection(db, 'spims_salary_records'),
              where('staffId', '==', tx.staffId),
              where('month', '==', month)
            );
            const salarySnap = await getDocs(salaryQ);
            if (salarySnap.empty) {
              await addDoc(collection(db, 'spims_salary_records'), {
                staffId: tx.staffId,
              ...
              await updateDoc(doc(db, 'spims_salary_records', salarySnap.docs[0].id), {
  ...
  const formatCategory = (cat: string) => {
    const map: Record<string, string> = {
      ...
      course_installment: 'Course Fee Installment',
      board_registration: 'Board Registration Fee',
      board_examination: 'Board Examination Fee',
      prospectus_sale: 'Prospectus/Form Sale',
      other_income: 'Other Income',
      staff_salary: 'Faculty/Staff Salary',
      building_rent: 'Building Rent',
      electricity: 'Electricity Bill',
      gas: 'Gas Bill',
      water: 'Water Bill',
      lab_supplies: 'Lab/Academic Supplies',
      food: 'Student Refreshments',
      maintenance: 'Campus Maintenance',
      transport: 'Transport / Fuel',
      equipment: 'Academic Equipment',
