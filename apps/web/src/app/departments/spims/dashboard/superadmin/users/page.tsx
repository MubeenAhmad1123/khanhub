...
export default function SuperAdminUserManagement() {
  ...
  // Form states
  const [adminForm, setAdminForm] = useState({ name: '', id: 'spims-ADM-001', pass: '' });
  const [cashierForm, setCashierForm] = useState({ id: 'spims-CSH-001', pass: '' });

  const fetchData = async () => {
    try {
      const qAdmins = query(collection(db, 'spims_users'), where('role', '==', 'admin'));
      const qCashier = query(collection(db, 'spims_users'), where('role', '==', 'cashier'), where('isActive', '==', true));
