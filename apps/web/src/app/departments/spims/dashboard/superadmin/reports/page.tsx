...
export default function SuperAdminReportsPage() {
  ...
  const handleGenerate = async () => {
    try {
      ...
      // === PATIENTS ===
      const activePatientsSnap = await getDocs(query(collection(db, 'spims_patients'), where('isActive', '==', true)));
      const totalActivePatients = activePatientsSnap.size;

      const newAdmissionsSnap = await getDocs(query(
        collection(db, 'spims_patients'),
        ...
        {generated && reportData && (
          <div id="spims-report-print" className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-10 space-y-10">
            ...
            <div className="text-center border-b border-gray-200 pb-6">
              <h2 className="text-2xl font-black text-gray-900">KhanHub spims Center — Super Admin Report</h2>
