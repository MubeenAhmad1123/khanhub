...
export default function SuperAdminDashboardPage() {
  ...
  const handleViewProfile = (u: any) => {
    if (u.role === 'patient') {
      router.push(`/departments/spims/dashboard/admin/patients/${u.id}`);
    } else if (u.role === 'staff' || u.role === 'admin' || u.role === 'cashier') {
      router.push(`/departments/spims/dashboard/admin/staff/${u.id}`);
    } else {
  ...
  return (
    <div className="space-y-8 pb-12 p-4 md:p-8 bg-gray-50 min-h-screen">
      ...
      {stats.pendingApprovals > 0 && (
        <Link href="/departments/spims/dashboard/superadmin/approvals"
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <ActionCard icon={<Users size={20} />} title="Manage Users" desc="Create admin or cashier roles" href="/departments/spims/dashboard/superadmin/users" color="teal" />
          <ActionCard icon={<CheckCircle size={20} />} title="Approval Queue" desc="Financial audit station" href="/departments/spims/dashboard/superadmin/approvals" color="amber" />
          <ActionCard icon={<Receipt size={20} />} title="Financial Ledger" desc="Global transaction history" href="/departments/spims/dashboard/admin/finance" color="blue" />
          <ActionCard icon={<Users size={20} />} title="Patient Registry" desc="All patient records history" href="/departments/spims/dashboard/admin/patients" color="indigo" />
        </div>
      </div>
      ...
      <div className="flex justify-center mt-12">
         <button onClick={() => { localStorage.removeItem('spims_session'); router.push('/departments/spims/login'); }}
