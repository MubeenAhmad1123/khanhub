    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 text-black">
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 no-print">
          ...
          {data && (
            <div className="flex gap-2 flex-wrap">
              <button onClick={handleDownload} className="flex items-center gap-2 bg-teal-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-teal-700 transition-colors">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4 no-print">
          <h2 className="font-bold text-gray-800 flex items-center gap-2"><Calendar className="w-5 h-5 text-teal-500" /> Select Month</h2>
        ...
        {data && (
          <div id="spims-payroll-print" ref={printRef} className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 text-center">
              ...
              <p className="text-base font-bold text-teal-700 mt-1">{data.monthLabel}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="bg-emerald-50 border border-emerald-200 p-5 rounded-2xl text-center shadow-sm">
                ...
                <div className="text-2xl font-black text-emerald-900">{formatPKR(data.totalNet)}</div>
              </div>
            </div>

            {/* Tabs */}
            <div className="flex bg-white rounded-2xl border border-gray-100 p-1 w-full no-print">
              <button onClick={() => setTab('salary')} className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all ${tab === 'salary' ? 'bg-teal-600 text-white shadow' : 'text-gray-500 hover:text-gray-800'}`}>
                Salary Sheet ({data.salaryRows.length} staff)
              </button>
              <button onClick={() => setTab('fines')} className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all ${tab === 'fines' ? 'bg-teal-600 text-white shadow' : 'text-gray-500 hover:text-gray-800'}`}>
            ...
            {tab === 'salary' && (
              <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
                ...
                <div className="flex justify-end no-print">
                  <div className="text-xs text-teal-700 bg-teal-50 px-3 py-1.5 rounded-lg font-medium flex items-center gap-1.5">
                ...
                <div className="overflow-x-auto rounded-xl border border-gray-200">
                  <table className="w-full text-sm border-collapse min-w-[850px]">
                    <thead className="bg-teal-50">
                      <tr>
                        <th className="px-4 py-3 text-left font-bold text-teal-800 border-b border-gray-200">#</th>
                        <th className="px-4 py-3 text-left font-bold text-teal-800 border-b border-gray-200">Staff Member</th>
                        <th className="px-4 py-3 text-left font-bold text-teal-800 border-b border-gray-200">Designation</th>
                        <th className="px-4 py-3 text-right font-bold text-teal-800 border-b border-gray-200">Gross Salary</th>
                        <th className="px-4 py-3 text-center font-bold text-teal-800 border-b border-gray-200">Absent Days</th>
                        <th className="px-4 py-3 text-right font-bold text-teal-800 border-b border-gray-200">Absent Ded.</th>
                        <th className="px-4 py-3 text-right font-bold text-teal-800 border-b border-gray-200">Fine Ded.</th>
                        <th className="px-4 py-3 text-right font-bold text-teal-800 border-b border-gray-200">Total Ded.</th>
                        <th className="px-4 py-3 text-right font-bold text-teal-800 border-b border-gray-200 bg-teal-100/70">Net Salary To Pay</th>
                        <th className="px-3 py-3 text-center font-bold text-teal-800 border-b border-gray-200 no-print">Breakdown</th>
                      </tr>
                    </thead>
                    <tbody>
                      {data.salaryRows.map((r: any, i: number) => (
                        <tr
                          ...
                          className="hover:bg-teal-50/60 transition-colors border-b border-gray-100 cursor-pointer group"
                        >
                          <td className="px-4 py-3.5 text-gray-400 font-mono text-xs">{i + 1}</td>
                          <td className="px-4 py-3.5 font-bold text-gray-900 group-hover:text-teal-700 transition-colors">{r.name}</td>
                          ...
                          <td className="px-4 py-3.5 text-right font-black text-teal-900 bg-teal-50 group-hover:bg-teal-100/80 transition-colors">
                            <span className="inline-block bg-teal-100 text-teal-800 px-2.5 py-1 rounded-lg border border-teal-200">
                          ...
                          <td className="px-3 py-3.5 text-center no-print">
                            <button
                              ...
                              className="p-1.5 bg-gray-100 hover:bg-teal-600 hover:text-white rounded-lg text-gray-500 transition-colors"
                      ...
                      <tr className="bg-teal-50 font-black text-xs">
                        <td colSpan={3} className="px-4 py-3.5 text-teal-800 uppercase tracking-wider">TOTAL</td>
                        <td className="px-4 py-3.5 text-right text-teal-800">{formatPKR(data.totalGross)}</td>
                        <td />
                        <td colSpan={3} className="px-4 py-3.5 text-right text-red-700">{formatPKR(data.totalDeductions)}</td>
                        <td className="px-4 py-3.5 text-right text-teal-950 font-black text-sm bg-teal-100/90">{formatPKR(data.totalNet)}</td>
            ...
            {tab === 'fines' && (
              <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 no-print">
                  <div className="flex items-center gap-3 flex-wrap">
                    <select
                      ...
                      className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-teal-500 text-black font-bold"
                    >
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-xl text-sm font-bold transition-colors"
                  >
                ...
                {/* Add Fine Form */}
                {showFineForm && (
                  <div className="bg-teal-50 border border-teal-100 rounded-2xl p-5 space-y-4 no-print">
                    <div className="flex items-center justify-between">
                      <h3 className="font-bold text-teal-800 flex items-center gap-2"><Receipt className="w-4 h-4" /> Add New Fine</h3>
                    ...
                    <button
                      ...
                      className="bg-teal-600 hover:bg-teal-700 text-white px-6 py-2.5 rounded-xl text-sm font-bold transition-colors disabled:opacity-60 flex items-center gap-2"
                    >
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full border border-gray-100 overflow-hidden my-8 transform transition-all">
            ...
            <div className="bg-teal-800 text-white p-6 relative">
              <button
                ...
                className="absolute top-5 right-5 text-teal-200 hover:text-white bg-teal-900/50 p-2 rounded-full transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
              <div className="flex items-center gap-3 mb-1">
                <span className="text-xs text-teal-200">{data?.monthLabel}</span>
              </div>
              <h2 className="text-2xl font-black tracking-tight">{selectedStaffModal.name}</h2>
              <p className="text-xs text-teal-200 mt-0.5">{selectedStaffModal.designation}</p>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="bg-teal-50 border border-teal-200 p-3.5 rounded-2xl text-center">
                  <div className="text-[10px] font-bold text-teal-700 uppercase tracking-wider mb-0.5">Net Salary to Pay</div>
                  <div className="text-base font-black text-teal-900">{formatPKR(selectedStaffModal.netPayable)}</div>
                  <div className="text-[10px] text-teal-600 font-bold mt-0.5">Final Payout</div>
                </div>
              </div>
              ...
              <div className="bg-teal-900 text-white rounded-2xl p-4 flex items-center justify-between">
                <div>
                  <div className="text-[11px] text-teal-200 font-medium">Final Money To Pay Staff</div>
                  <div className="text-xs text-teal-300">Base Salary - Total Deductions</div>
                </div>
            ...
            <div className="bg-gray-50 border-t border-gray-100 px-6 py-4 flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-5 py-2 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl text-xs transition-colors"
              >
