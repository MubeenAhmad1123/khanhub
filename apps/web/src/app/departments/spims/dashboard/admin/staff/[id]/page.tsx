...
import type { StaffMember, AttendanceRecord, StaffFine, LeaveRecord } from '@/types/spims';
import {
...
export default function StaffDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const staffDoc = await getDoc(doc(db, 'spims_staff', id));
      ...
      const [attSnap, fineSnap, leaveSnap] = await Promise.all([
        getDocs(query(collection(db, 'spims_attendance'), where('staffId', '==', id))),
        getDocs(query(collection(db, 'spims_fines'), where('staffId', '==', id))),
        getDocs(query(collection(db, 'spims_leaves'), where('staffId', '==', id))),
      ]);
  ...
  const handleAddLeave = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, 'spims_leaves'), {
        staffId: id,
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm space-y-6 print:shadow-none print:border-none print:p-0">
            ...
            <div className="hidden print:block pt-12 text-center border-t border-gray-100 mt-12">
               <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Generated by KhanHub Spims Portal — {new Date().toLocaleString()}</p>
            </div>
          </div>
        </div>
      )}
