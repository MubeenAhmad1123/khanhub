  ...
  const fetchPatients = async () => {
    try {
      ...
      const [snap, feesSnap, canteenSnap] = await Promise.all([
        getDocs(collection(db, 'spims_patients')),
        getDocs(query(collection(db, 'spims_fees'), where('month', '==', currentMonth))),
        getDocs(query(collection(db, 'spims_canteen'), where('month', '==', currentMonth)))
      ]);
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <Link 
            href="/departments/spims/dashboard/admin/patients/new"
        ...
        {filteredPatients.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredPatients.map(patient => (
              <Link 
                href={`/departments/spims/dashboard/admin/patients/${patient.id}`} 
                key={patient.id}
