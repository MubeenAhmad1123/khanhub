...
export default function AdminStaffPage() {
  ...
  const fetchData = async () => {
    try {
      const staffSnap = await getDocs(collection(db, 'spims_staff'));
      ...
      const attendanceSnap = await getDocs(collection(db, 'spims_attendance'));
  ...
  const handleCreateStaff = async (e: React.FormEvent) => {
    ...
    if (photoFile) {
      try {
        photoUrl = await uploadToCloudinary(photoFile, 'khanhub/spims/staff');
      } catch (err) {
    ...
    if (res.success) {
      ...
      setShowModal(false);
      setForm({ name: '', gender: 'male', customId: 'spims-STF-001', password: '', staffRole: '', phone: '', salary: '', duties: [] });
              ...
              return (
                <div key={s.id} className="bg-white rounded-[2rem] p-5 border border-gray-100 shadow-sm space-y-4 transition-all active:scale-[0.99]">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center border border-gray-100 overflow-hidden shadow-inner flex-shrink-0">
                        {s.photoUrl 
                          ? <img src={s.photoUrl} alt={s.name || 'Staff'} className="w-full h-full object-cover" />
                  ...
                  <div className="flex items-center justify-between gap-3 pt-2">
                    ...
                    <button 
                      onClick={() => router.push(`/departments/spims/dashboard/admin/staff/${s.id}`)}
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleCreateStaff} className="p-6 space-y-4 overflow-y-auto flex-1">
              ...
              <p className="text-[10px] text-gray-400 bg-gray-50 rounded-xl px-4 py-3">
                Staff logs in at <span className="font-mono font-bold">/departments/spims/login</span> using their ID + password.
              </p>
