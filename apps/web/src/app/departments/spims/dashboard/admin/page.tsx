    ...
    <div className="space-y-4 md:space-y-6 pb-10 w-full overflow-x-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <Link
            href={session?.role === 'superadmin' ? "/departments/spims/dashboard/superadmin/reports" : "/departments/spims/dashboard/admin/reports"}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
        <div className="bg-white rounded-2xl md:rounded-[2.5rem] border border-gray-100 shadow-sm p-4 md:p-8 flex items-center gap-4 md:gap-6 group hover:border-teal-100 transition-colors">
          ...
          <div>
            ...
            <div className="text-[9px] md:text-sm text-gray-500 font-black uppercase tracking-widest mt-1">Total Student</div>
          </div>
        </div>
              ...
              <div className="text-center py-10">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {activeStudents.map(student => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center text-black font-black text-sm group-hover:bg-teal-50 group-hover:text-teal-600 transition-colors flex-shrink-0">
                          {student.name?.[0]?.toUpperCase()}
                        </div>
                        <div>
                          <p className="font-bold text-gray-900 text-sm">{student.name}</p>
                          <p className="text-[9px] font-mono text-gray-500 font-bold uppercase tracking-widest">{student.rollNo || student.id}</p>
                        </div>
                      </div>
                      <ChevronRight size={16} className="text-gray-400 group-hover:text-teal-500 group-hover:translate-x-1 transition-all flex-shrink-0" />
                    </Link>
                  ))}
                </div>
            )}
