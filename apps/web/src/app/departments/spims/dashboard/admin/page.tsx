  ...
  const loadDashboard = async () => {
    try {
      ...
      const [
        ...
        getDocs(query(collection(db, 'spims_patients'), where('isActive', '==', true))),
        getDocs(query(collection(db, 'spims_staff'), where('isActive', '==', true))),
        getDocs(query(
        ...
        getDocs(query(collection(db, 'spims_transactions'), orderBy('createdAt', 'desc'), limit(5))),
        getDocs(query(collection(db, 'spims_contributions'), orderBy('createdAt', 'desc'), limit(5))),
      ]);
      ...
      await Promise.all(
        [...new Set(contribData.map((c: any) => c.staffId).filter(Boolean))].map(async (sid) => {
          const snap = await getDocs(query(collection(db, 'spims_staff'), where('loginUserId', '==', sid)));
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <Link href="/departments/spims/dashboard/admin/finance" className="text-xs text-teal-600 font-medium flex items-center gap-1 hover:underline">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: 'Add Patient', href: '/departments/spims/dashboard/admin/patients/new', icon: <Plus className="w-4 h-4" />, color: 'bg-teal-50 text-teal-700 hover:bg-teal-100' },
                { label: 'View Staff', href: '/departments/spims/dashboard/admin/staff', icon: <UserCog className="w-4 h-4" />, color: 'bg-blue-50 text-blue-700 hover:bg-blue-100' },
                { label: 'Finance Log', href: '/departments/spims/dashboard/admin/finance', icon: <TrendingUp className="w-4 h-4" />, color: 'bg-green-50 text-green-700 hover:bg-green-100' },
                { label: 'Reports', href: '/departments/spims/dashboard/admin/reports', icon: <BarChart3 className="w-4 h-4" />, color: 'bg-purple-50 text-purple-700 hover:bg-purple-100' },
              ].map(link => (
                <Link
                  key={link.href}
                  href={link.href}
