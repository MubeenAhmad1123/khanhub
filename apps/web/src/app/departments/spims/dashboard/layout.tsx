...
type spimsRole = 'superadmin' | 'admin' | 'cashier' | 'staff' | 'family';
...
const NAV_ITEMS: NavItem[] = [
  { label: 'Overview',        href: '/departments/spims/dashboard/superadmin',          icon: <LayoutDashboard size={16}/>, roles: ['superadmin'] },
  { label: 'User Management', href: '/departments/spims/dashboard/superadmin/users',     icon: <Users size={16}/>,           roles: ['superadmin'] },
  { label: 'Approvals',       href: '/departments/spims/dashboard/superadmin/approvals', icon: <CheckCircle size={16}/>,     roles: ['superadmin'] },
  { label: 'Audit Log',       href: '/departments/spims/dashboard/superadmin/audit',     icon: <Activity size={16}/>,        roles: ['superadmin'] },
  { label: 'Overview',        href: '/departments/spims/dashboard/admin',                icon: <LayoutDashboard size={16}/>, roles: ['admin'] },
  { label: 'Students',        href: '/departments/spims/dashboard/admin/patients',       icon: <Users size={16}/>,           roles: ['admin', 'superadmin'] },
  { label: 'Faculty & Staff', href: '/departments/spims/dashboard/admin/staff',          icon: <UserCog size={16}/>,         roles: ['admin', 'superadmin'] },
  { label: 'Accounts',        href: '/departments/spims/dashboard/admin/finance',        icon: <Banknote size={16}/>,        roles: ['admin', 'superadmin'] },
  { label: 'Academic Reports', href: '/departments/spims/dashboard/admin/reports',       icon: <FileBarChart size={16}/>,    roles: ['admin', 'superadmin'] },
  { label: 'Finance Station', href: '/departments/spims/dashboard/cashier',              icon: <CreditCard size={16}/>,      roles: ['cashier'] },
  { label: 'My Attendance',   href: '/departments/spims/dashboard/staff',               icon: <CalendarDays size={16}/>,    roles: ['staff'] },
  { label: 'Student Dashboard', href: '/departments/spims/dashboard/family',             icon: <User size={16}/>,            roles: ['family'] },
  { label: 'My Profile',      href: '/departments/spims/dashboard/profile',             icon: <User size={16}/>,            roles: ['superadmin', 'admin', 'cashier', 'staff', 'family'] },
];

const ROLE_COLORS: Record<spimsRole, string> = {
...
const ROLE_LABELS: Record<spimsRole, string> = {
...
export default function SpimsDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const { session: user, loading: isChecking, signOut: handleSignOut } = useSpimsSession();
  ...
  const navItems = NAV_ITEMS.filter(item => user && item.roles.includes(user.role as spimsRole));
  const role = user?.role as spimsRole;
