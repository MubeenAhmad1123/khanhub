...
const INCOME_CATEGORIES = [
  { value: 'admission_fee',      label: 'Admission Fee',          needsPatient: true,  needsStaff: false },
  { value: 'course_installment', label: 'Course Fee Installment', needsPatient: true,  needsStaff: false },
  { value: 'board_registration', label: 'Board Registration Fee', needsPatient: true,  needsStaff: false },
  { value: 'board_examination',  label: 'Board Examination Fee',  needsPatient: true,  needsStaff: false },
  { value: 'prospectus_sale',    label: 'Prospectus/Form Sale',   needsPatient: false, needsStaff: false },
  { value: 'other_income',       label: 'Other Income',           needsPatient: false, needsStaff: false },
];

const EXPENSE_CATEGORIES = [
  { value: 'staff_salary',     label: 'Faculty/Staff Salary',   needsPatient: false, needsStaff: true },
  { value: 'building_rent',    label: 'Building Rent',          needsPatient: false, needsStaff: false },
  ...
  { value: 'lab_supplies',     label: 'Lab/Academic Supplies',  needsPatient: false, needsStaff: false },
  { value: 'food',             label: 'Student Refreshments',   needsPatient: false, needsStaff: false },
  { value: 'maintenance',      label: 'Campus Maintenance',     border: false,       needsPatient: false, needsStaff: false },
  { value: 'marketing',        label: 'Marketing & Printing',   needsPatient: false, needsStaff: false },
  { value: 'transport',        label: 'Transport',              needsPatient: false, needsStaff: false },
  { value: 'equipment',        label: 'Academic Equipment',     needsPatient: false, needsStaff: false },
...
export default function SpimsCashierDashboard() {
  ...
  async function loadPatients() {
    try {
      ...
      const list = snap.docs.map(d => ({
        ...
        admissionDate: d.data().enrollmentDate || d.data().admissionDate || null,
        customId: d.data().rollNumber || d.id.slice(0, 8),
      }));
  ...
  async function loadStaff() {
    try {
      const snap = await getDocs(collection(db, 'spims_staff'));
      const list = snap.docs.map(d => ({
        id: d.id,
        name: d.data().name || 'Unknown',
  ...
  async function loadTodayEntries() {
    try {
      ...
      const snap = await getDocs(
        query(collection(db, 'spims_transactions'),
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await addDoc(collection(db, 'spims_transactions'), txData);
          ...
          {needsPatient && (
            <div className="relative">
              ...
              <p className="text-xs text-gray-400 mt-1 ml-1">
                Type any letter to search from {allPatients.length} students
              </p>
              {selectedPatient && (
