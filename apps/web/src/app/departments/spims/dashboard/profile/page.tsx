...
export default function ProfilePage() {
  ...
  const { sections, loading: visibilityLoading } = useVisibleSections('spims', 'staff', session?.uid || '');
  ...
  const inputStyle = "bg-gray-50 border-gray-100 rounded-xl px-4 py-3 w-full border focus:ring-2 focus:ring-teal-500/20 outline-none text-sm transition-all text-gray-800";
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        setLoading(true);
        const userRef = doc(db, 'spims_users', parsed.uid);
        ...
        const fallbackSnap = await getDocs(query(collection(db, 'spims_staff'), where('loginUserId', '==', parsed.uid)));
        let sData = null;
        ...
        const primaryDept = merged.department || 'spims';
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900">SPIMS Academy</h1>
          ...
          <div className="flex items-center gap-3">
             ...
             <button 
               ...
               className="p-2.5 text-gray-400 hover:text-teal-600 hover:bg-teal-50 rounded-xl transition-all tooltip" 
               title="Sign Out"
             >
               <LogOut size={20} />
             </button>
          ...
          <div className={`${cardStyle} p-8 text-center relative overflow-hidden`}>
            ...
            {sections.designation !== false && (
              <p className="text-teal-600 font-semibold text-xs uppercase tracking-wider mt-1 bg-teal-50 px-3 py-1 rounded-full inline-block">
                {profile?.designation || 'Academic Staff'}
              </p>
            )}
              ...
              <div ref={el => { sectionRefs.current['score'] = el; }} id="section-score" className={`${cardStyle} p-6 scroll-mt-[140px]`}>
                 <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 pb-4 border-b border-gray-50">
                  ...
                  <input
                    ...
                    className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 text-sm font-bold text-gray-700 outline-none focus:ring-2 focus:ring-teal-500/20 w-full sm:w-auto"
                  />
                </div>
            ...
            {/* --- IDENTITY SECTION --- */}
            {
              <div ref={el => { sectionRefs.current['profile'] = el; }} id="section-profile" className={`${cardStyle} p-6 scroll-mt-[140px]`}>
                ...
                <div className="mt-8 bg-gray-50 border border-gray-100 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-4">
                   <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-white rounded-xl border border-gray-100 flex items-center justify-center text-teal-600">
                        <Award size={24} />
                      </div>
                      <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200 text-gray-900">
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <div>
                <h3 className="text-sm font-black uppercase tracking-widest text-teal-500">Salary Breakdown</h3>
            ...
            <div className="p-8 overflow-y-auto space-y-6 flex-1 text-xs">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-teal-50/30 rounded-2xl border border-teal-100/50">
                  <p className="text-[9px] font-black text-teal-500 uppercase tracking-widest mb-1">Monthly Base Salary</p>
