...
import { createSpimsUserServer, markSetupComplete } from '../actions/createSpimsUser';
...
export default function SetupPage() {
  ...
  // form state
  const [superAdminId, setSuperAdminId] = useState('SPIMS-SA-001');
  const [superAdminPassword, setSuperAdminPassword] = useState('');
  const [confirmPassword, setConfirmPassword] = useState('');
  const [cashierId, setCashierId] = useState('SPIMS-FIN-001');
  ...
  useEffect(() => {
    if (!mounted) return;
    import('../actions/createSpimsUser').then(m => {
  ...
  useEffect(() => {
    ...
    async function checkSetup() {
      try {
        const snap = await getDoc(doc(db, 'spims_meta', 'setup'));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const saResult = await createSpimsUserServer(
      ...
      // 2. Create cashier
      const cshResult = await createSpimsUserServer(
        cashierId, cashierPassword, 'cashier', 'Finance Officer'
      );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-gray-50">
        <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-xl p-10 border border-gray-100">
          <h1 className="text-4xl font-black text-center text-gray-900 mb-2 tracking-tight">SPIMS System Setup</h1>
          <p className="text-xs text-center tracking-[0.3em] text-gray-400 font-black uppercase mb-10">
            Initialize College Portal
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              <h2 className="text-lg font-black text-gray-800 mb-4 border-b border-gray-50 pb-2">Finance Office Account</h2>
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] 
                                    text-gray-400 block mb-1 ml-2">Finance ID</label>
                  <input
                    type="text"
                    value={cashierId}
