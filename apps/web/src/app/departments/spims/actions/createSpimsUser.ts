...
function getAdminApp(): App {
  const existing = getApps().find(a => a.name === 'spims-admin');
...
export async function createSpimsUserServer(
  ...
  try {
    ...
    try {
      ...
      await adminDb.collection('spims_users').doc(existingUser.uid).delete();
    } catch {
...
export async function createStaffMemberServer(
  ...
  try {
    ...
    const adminAuth = getAuth(app);
    const adminDb = getFirestore(app);
    const email = `${customId.toLowerCase()}@spims.khanhub`;
    ...
    await adminDb.collection('spims_users').doc(userRecord.uid).set({
      customId,
      role: 'staff',
      displayName,
      isActive: true,
    ...
    const staffRef = adminDb.collection('spims_staff').doc();
    await staffRef.set({
      name: displayName,
      gender: gender || 'male',
      role: staffRole,
