...
export default function SukoonDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (isChecking) {
    return (
      <div className="min-h-screen bg-[#FDF2F8] dark:bg-[#0A0A0A] flex items-center justify-center">
        <div className="flex flex-col items-center gap-6">
          <div className="relative w-16 h-16">
            <div className="absolute inset-0 border-4 border-purple-500/20 rounded-full" />
            <div className="absolute inset-0 border-4 border-purple-500 border-t-transparent rounded-full animate-spin" />
          </div>
          <p className="text-purple-600 dark:text-purple-400 text-xs font-black uppercase tracking-[0.3em] animate-pulse italic">Healing Portal...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col h-full bg-white/50 dark:bg-black/50 backdrop-blur-xl border-r border-gray-200/50 dark:border-white/5">
        ...
        <div className="p-8">
          ...
          >
            <div className="w-6 h-6 rounded-full border border-gray-200 dark:border-white/10 flex items-center justify-center group-hover:border-purple-500/50 transition-colors">
                    ...
                    {Object.entries(DEPT_INFO).map(([key, info]) => (
                      ...
                      >
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
                          pathname.includes(key) ? 'bg-white/20' : 'bg-gray-100 dark:bg-white/5 group-hover:bg-purple-500/10'
                        }`}>
            ...
            <div className="flex p-1.5 bg-gray-100/50 dark:bg-white/5 rounded-2xl border border-gray-200 dark:border-white/5 mb-8">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[10px] font-black transition-all ${
                  viewMode === 'dept' ? 'bg-white dark:bg-white/10 shadow-sm text-purple-600 dark:text-purple-400' : 'text-gray-400 hover:text-gray-600'
                }`}
              >
                LOCAL
              </button>
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[10px] font-black transition-all ${
                  viewMode === 'hq' ? 'bg-white dark:bg-white/10 shadow-sm text-purple-600 dark:text-purple-400' : 'text-gray-400 hover:text-gray-600'
                }`}
              >
                CORE
              </button>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-4 px-5 py-4 rounded-[1.5rem] text-sm transition-all relative group overflow-hidden ${
                  isActive 
                    ? 'bg-gradient-to-r from-purple-500/10 to-transparent text-purple-600 dark:text-purple-400' 
      ...
      <div className="flex-1 lg:ml-72 flex flex-col min-h-screen relative">
        ...
        <header className="lg:hidden sticky top-0 z-20 bg-white/80 dark:bg-black/80 backdrop-blur-xl border-b border-gray-100 dark:border-white/5 px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex flex-col items-center">
            <span className="font-black text-xs uppercase tracking-[0.2em] dark:text-white">Sukoon Node</span>
