...
export default function SukoonDashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
      <div className="min-h-screen bg-[#FCFAF2] flex items-center justify-center">
        <div className="flex flex-col items-center gap-6">
          ...
          <p className="text-purple-600 text-xs font-black uppercase tracking-[0.3em] animate-pulse italic">Healing Portal...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-col h-full bg-white border-r border-gray-200">
        ...
        <div className="p-8">
          <Link 
            ...
            className="flex items-center gap-2 text-gray-400 hover:text-purple-600 text-[10px] font-bold mb-8 transition-all group uppercase tracking-widest"
          >
            <div className="w-6 h-6 rounded-full border border-gray-200 flex items-center justify-center group-hover:border-purple-500/50 transition-colors">
          ...
          <div className="flex items-center gap-4 mb-8">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-2xl transition-all duration-500 ${
              viewMode === 'hq' ? 'bg-gradient-to-br from-indigo-500 to-purple-600 rotate-3' : 'bg-gradient-to-br from-purple-500 to-pink-600 -rotate-3'
            }`}>
              {viewMode === 'hq' ? <Shield size={24} /> : <Heart size={24} />}
            </div>
            <div>
              <p className="font-black text-lg leading-tight tracking-tight text-gray-900">
                {viewMode === 'hq' ? 'HQ Admin' : 'Sukoon Dept'}
              </p>
              <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mt-0.5">
                {viewMode === 'hq' ? 'System Core' : 'Comfort Grid'}
              </p>
            </div>
          </div>
            ...
            <div className="relative mb-6">
              ...
              >
                <div className="flex items-center gap-3">
                  <ExternalLink size={14} className="text-purple-500 group-hover:rotate-12 transition-transform" />
              ...
              {portalOpen && (
                <div className="absolute top-full left-0 right-0 mt-3 z-50 rounded-3xl border border-gray-200 bg-white backdrop-blur-2xl shadow-2xl p-3 animate-in fade-in slide-in-from-top-2 duration-300 overflow-hidden">
                    ...
                    {Object.entries(DEPT_INFO).map(([key, info]) => (
                      ...
                      >
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
                          pathname.includes(key) ? 'bg-white/20' : 'bg-gray-100 group-hover:bg-purple-500/10'
                        }`}>
            ...
            <div className="flex p-1.5 bg-gray-100/50 rounded-2xl border border-gray-200 mb-8">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[10px] font-black transition-all ${
                  viewMode === 'dept' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-400 hover:text-gray-600'
                }`}
              >
                LOCAL
              </button>
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[10px] font-black transition-all ${
                  viewMode === 'hq' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-400 hover:text-gray-600'
                }`}
              >
                CORE
              </button>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-4 px-5 py-4 rounded-[1.5rem] text-sm transition-all relative group overflow-hidden ${
                  isActive 
                    ? 'bg-gradient-to-r from-purple-500/10 to-transparent text-purple-600' 
                    : 'text-gray-400 hover:text-gray-900 hover:bg-gray-50/50'
                }`}
              >
                {isActive && (
                  <div className="absolute left-0 top-1/4 bottom-1/4 w-1 bg-purple-500 rounded-r-full shadow-[0_0_12px_rgba(168,85,247,0.5)]" />
                )}
                <div className={`transition-transform duration-500 group-hover:scale-110 ${isActive ? 'text-purple-500' : ''}`}>
                  {item.icon}
                </div>
  ...
  return (
    <div className="min-h-screen flex bg-[#FCFAF2] text-black font-sans">
      {/* Sidebar Desktop */}
      ...
      {/* Main Content Area */}
      <div className="flex-1 lg:ml-72 flex flex-col min-h-screen relative bg-[#FCFAF2]">
        {/* Background Decorative Elements */}
        <div className="fixed top-0 right-0 w-[500px] h-[500px] bg-purple-500/5 rounded-full blur-[120px] -mr-64 -mt-64 pointer-events-none" />
        <div className="fixed bottom-0 left-0 w-[500px] h-[500px] bg-pink-500/5 rounded-full blur-[120px] -ml-64 -mb-64 pointer-events-none" />

        {/* Mobile Header */}
        <header className="lg:hidden sticky top-0 z-20 bg-white/80 backdrop-blur-xl border-b border-gray-100 px-6 py-4 flex items-center justify-between">
          <button 
          ...
          <div className="flex flex-col items-center">
            <span className="font-black text-xs uppercase tracking-[0.2em] text-gray-900">Sukoon Node</span>
            <span className="text-[8px] font-bold text-purple-500 uppercase tracking-widest mt-0.5">Premium Comfort</span>
          </div>
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-purple-400 to-pink-500 flex items-center justify-center text-white text-xs font-black shadow-lg shadow-purple-500/20">
            {user?.displayName?.[0]}
          </div>
        </header>

        {/* Desktop Header */}
        <header className="hidden lg:flex sticky top-0 z-20 bg-[#FCFAF2]/80 backdrop-blur-xl border-b border-gray-100 px-12 py-6 items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="flex flex-col">
              <h2 className="text-xs font-black uppercase tracking-[0.4em] text-gray-400">Khan Hub Network</h2>
              <p className="text-[10px] font-bold text-purple-500 uppercase tracking-widest mt-1">Sukoon Department Portal</p>
            </div>
            
            <div className="h-8 w-px bg-gray-200" />

            <div className="text-[11px] font-black uppercase tracking-[0.1em] text-gray-400 flex items-center gap-2">
          ...
          <div className="flex items-center gap-8">
             {user?.uid && <StaffNotifications uid={user.uid} dept="sukoon" />}
             ...
             <div className="flex items-center gap-4 pl-2">
                <div className="flex flex-col items-end">
                   ...
                   <span className="px-2 py-0.5 rounded-lg bg-purple-500/10 text-purple-500 text-[8px] font-black uppercase tracking-wider mt-1 border border-purple-500/20">
        ...
        <main className="flex-1 p-6 lg:p-12 bg-[#FCFAF2] text-black transition-all duration-1000">
