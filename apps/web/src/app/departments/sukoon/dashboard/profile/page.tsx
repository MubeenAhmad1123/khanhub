...
export default function ProfilePage() {
  ...
  const fetchMetrics = useCallback(async (sId: string) => {
    try {
      const [attSnap, dutySnap, dressSnap, pointsSnap, taskSnap, fineSnap] = await Promise.all([
        getDocs(query(collection(db, `sukoon_attendance`), where('staffId', '==', sId), orderBy('date', 'desc'), limit(30))),
        getDocs(query(collection(db, `sukoon_duty_logs`), where('staffId', '==', sId), orderBy('date', 'desc'), limit(30))),
        getDocs(query(collection(db, `sukoon_dress_logs`), where('staffId', '==', sId), orderBy('date', 'desc'), limit(30))),
        getDocs(query(collection(db, `sukoon_growth_points`), where('staffId', '==', sId), limit(1))),
        getDocs(query(collection(db, `sukoon_special_tasks`), where('staffId', '==', sId), orderBy('date', 'desc'), limit(30))),
        getDocs(query(collection(db, `sukoon_fines`), where('staffId', '==', sId), orderBy('date', 'desc'), limit(30)))
      ]);
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        setLoading(true);
        const userSnap = await getDoc(doc(db, 'sukoon_users', parsed.uid));
        if (!userSnap.exists()) { router.push('/departments/sukoon/login'); return; }
        ...
        if (parsed.role === 'family' && uData.patientId) {
          ...
          let sSnap = await getDocs(query(collection(db, 'sukoon_staff'), where('loginUserId', '==', parsed.uid)));
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-[#f0f2f5]">
      <div className="animate-spin text-teal-600"><Loader2 size={40} /></div>
    </div>
  );

  return (
    <div className="min-h-screen bg-[#f0f2f5] pb-24 text-slate-800 font-sans selection:bg-teal-100">
      <div className="sticky top-0 z-50 px-4 py-6 md:px-12 bg-[#f0f2f5]/80 backdrop-blur-md border-b border-white/20">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-2xl ${neumorphicOutset} bg-[#f0f2f5] text-teal-600`}>
              <Activity size={28} className="drop-shadow-sm" />
            </div>
            <div>
              <h1 className="text-2xl md:text-3xl font-black tracking-tight text-slate-900">Sukoon Portal</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4 space-y-8">
          <div className={`p-8 rounded-[3rem] ${glassStyle} flex flex-col items-center relative overflow-hidden`}>
            <div className="absolute -top-12 -right-12 w-32 h-32 bg-teal-500/10 rounded-full blur-3xl" />
            <div className="relative group">
              ...
              <button onClick={() => fileRef.current?.click()} className={`absolute -bottom-2 -right-2 p-3 rounded-2xl bg-teal-600 text-white shadow-xl hover:scale-110 transition-transform`}><Camera size={18} /></button>
            ...
            <p className="text-teal-600 font-bold text-sm tracking-wide uppercase mt-1">{profile?.designation || (session?.role === 'family' ? 'Family Member' : 'Care Staff')}</p>
        ...
        <div className="lg:col-span-8 space-y-8">
          {session?.role === 'family' ? (
             <div className={`p-8 md:p-10 rounded-[3.5rem] ${glassStyle}`}>
                <h3 className="text-2xl font-black text-slate-900 mb-8 flex items-center gap-3"><Heart className="text-rose-500" /> Linked Patient Details</h3>
                {patientDoc ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#f0f2f5]`}>
                      <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Patient Name</p>
                      <p className="text-lg font-black text-slate-900">{patientDoc.name}</p>
                    </div>
                    <div className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#f0f2f5]`}>
                      <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Admission Date</p>
                      <p className="text-lg font-black text-slate-900">{formatDateDMY(patientDoc.admissionDate)}</p>
                    </div>
                    <div className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#f0f2f5]`}>
                      <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Status</p>
                      <span className={`px-4 py-1 rounded-xl text-[10px] font-black uppercase ${patientDoc.isActive ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                        {patientDoc.isActive ? 'Active' : 'Inactive'}
                      </span>
                    </div>
                  </div>
                ) : (
                  <p className="text-slate-400 italic">No patient profile linked to this user.</p>
                )}
             </div>
          ) : (
            <>
              <div className={`p-2 rounded-[2rem] ${neumorphicOutset} bg-[#f0f2f5] flex overflow-x-auto no-scrollbar gap-1`}>
                {[
                  ...
                  <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex items-center gap-2 px-6 py-4 rounded-[1.5rem] text-sm font-black transition-all whitespace-nowrap ${activeTab === tab.id ? `bg-teal-600 text-white shadow-[4px_4px_10px_rgba(20,184,166,0.3)] scale-105` : `text-slate-400 hover:text-slate-600 hover:bg-slate-200/50`}`}>
                    {tab.icon} {tab.label}
                  </button>
                ))}
              </div>
                    ...
                    <div className="space-y-4">
                      {specialTasks.map(task => (
                        <div key={task.id} className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#f0f2f5] flex items-center justify-between group hover:scale-[1.01] transition-transform`}>
                          ...
                          <div className="px-3 py-1 rounded-xl bg-white border border-slate-100 text-[10px] font-black text-teal-600">+{task.points} PTS</div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {activeTab === 'finance' && (
                  <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                      <div className={`p-8 rounded-[3rem] ${neumorphicOutset} bg-teal-600 text-white`}><p className="text-xs font-black uppercase opacity-60">Estimated Payable</p><h4 className="text-4xl font-black mt-2">Rs. {totalEarnings.toLocaleString()}</h4></div>
                     ...
                     <div className="space-y-4">
                        {duties.map(duty => (
                          <div key={duty.id} className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#f0f2f5] flex items-center justify-between`}>
                            ...
                            <div className="px-3 py-1 rounded-xl bg-white border border-slate-100 text-[10px] font-black text-teal-600">+{duty.points} PTS</div>
                          </div>
                        ))}
                     </div>
                  </div>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {dressLogs.map(log => (
                          <div key={log.id} className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#f0f2f5] flex items-center justify-between`}>
                            <div className="flex items-center gap-4"><div className={`p-3 rounded-2xl ${log.status === 'yes' ? 'bg-teal-100 text-teal-600' : 'bg-slate-100 text-slate-400'}`}><Shirt size={20} /></div><div><p className="font-black text-slate-900">{formatDateDMY(log.date)}</p><p className="text-[10px] font-black text-slate-400">{log.status === 'yes' ? 'Compliant' : 'Non-Compliant'}</p></div></div>
                            {log.status === 'yes' && <div className="px-3 py-1 rounded-xl bg-white border border-slate-100 text-[10px] font-black text-teal-600">+{log.points} PTS</div>}
                          </div>
                        ))}
                     </div>
                  </div>
                  ...
                  <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       <div className="space-y-6">
                          <div><p className="text-[10px] font-black uppercase text-slate-400 mb-2 px-2">National ID (CNIC)</p><div className={`p-5 rounded-[2rem] ${neumorphicInset} bg-white flex items-center gap-4`}><Shield className="text-teal-500" size={20} /><p className="text-sm font-bold text-slate-900">{profile?.cnic || 'Not provided'}</p></div></div>
                          <div><p className="text-[10px] font-black uppercase text-slate-400 mb-2 px-2">Home Address</p><div className={`p-5 rounded-[2rem] ${neumorphicInset} bg-white flex items-start gap-4`}><MapPin className="text-teal-500 mt-1" size={20} /><p className="text-sm font-bold text-slate-900 leading-relaxed">{profile?.address || 'No address registered.'}</p></div></div>
                       </div>
                       <div className="space-y-6">
                          <div><p className="text-[10px] font-black uppercase text-slate-400 mb-2 px-2">Work Schedule</p><div className={`p-5 rounded-[2rem] ${neumorphicInset} bg-white flex items-center gap-4`}><Clock className="text-teal-500" size={20} /><p className="text-sm font-bold text-slate-900">{profile?.dutyStartTime || '09:00'} - {profile?.dutyEndTime || '17:00'}</p></div></div>
                       </div>
                    </div>
                  </div>
                )}
