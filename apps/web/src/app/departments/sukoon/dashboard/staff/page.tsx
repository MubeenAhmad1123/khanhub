...
export default function StaffSelfPage() {
  ...
  const { sections, loading: visibilityLoading } = useVisibleSections('sukoon', 'staff', staffProfile?.id || user?.uid || '');

  const fetchData = useCallback(async () => {
    if (!user) return;
    try {
      // 1. Fetch Staff Profile
      const staffRef = doc(db, 'sukoon_users', user.uid);
      let staffSnap = await getDoc(staffRef);
      
      if (!staffSnap.exists()) {
        const fallbackSnap = await getDocs(query(collection(db, 'sukoon_staff'), where('loginUserId', '==', user.uid)));
      ...
      const contribSnap = await getDocs(
        query(
          collection(db, 'sukoon_contributions'), 
          where('staffId', '==', staffId),
      ...
      const tasksSnap = await getDocs(
        query(
          collection(db, 'sukoon_special_tasks'), 
          where('staffId', '==', staffId),
  ...
  const handleTaskUpdate = async (taskId: string, newStatus: string) => {
    try {
      await updateDoc(doc(db, 'sukoon_special_tasks', taskId), {
        status: newStatus,
