...
import type { AttendanceRecord, StaffContribution, StaffMember } from '@/types/job-center';
import {
...
export default function StaffSelfPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const staffSnap = await getDocs(
        query(collection(db, 'jobcenter_staff'), where('loginUserId', '==', user.uid))
      );
      ...
      const attSnap = await getDocs(
        query(collection(db, 'jobcenter_attendance'), where('staffId', '==', staffId), where('date', '==', today))
      );
      ...
      const contribSnap = await getDocs(
        query(collection(db, 'jobcenter_contributions'), where('staffId', '==', staffId))
      );
      setContributions(
        contribSnap.docs
          .map(d => ({ 
  ...
  useEffect(() => {
    ...
    if (!user || user.role !== 'staff') { router.push('/departments/job-center/login'); return; }
    fetchData();
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] overflow-x-hidden w-full max-w-full pb-24">
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        {/* Check In / Out Card */}
        <div className={`rounded-2xl p-4 border-l-4 ${checkedIn || checkedOut ? 'border-orange-500 bg-white/5' : 'border-amber-500 bg-white/5'}`}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className={`p-4 rounded-2xl text-center ${checkedIn ? 'bg-orange-500/10' : 'bg-white/5'}`}>
              <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">Check In</p>
              <p className={`font-mono text-2xl font-black ${checkedIn ? 'text-orange-400' : 'opacity-30'}`}>
          ...
          {checkedOut || isOverridden ? (
            ...
            <button
              ...
              className="w-full py-5 rounded-2xl font-black text-sm uppercase tracking-wide flex items-center justify-center gap-3 transition-all active:scale-95 shadow-lg shadow-orange-500/20 disabled:opacity-50 disabled:cursor-not-allowed bg-orange-500 text-white animate-pulse"
            >
              {checkLoading
            ...
            <ol className="space-y-2">
              {staffProfile?.duties?.map((d: any, i: number) => (
                <li key={d.id || i} className="flex items-start gap-3 p-3 bg-white/5 rounded-xl">
                  <span className="w-5 h-5 rounded-lg bg-orange-500/10 text-orange-400 text-[10px] font-black flex items-center justify-center flex-shrink-0 mt-0.5">{i + 1}</span>
        ...
        <div className="bg-white/5 rounded-2xl border border-white/10 p-4">
          ...
          <p className="text-slate-400 text-xs mb-4">Share what you accomplished, any feedback, or ideas for improving the job center.</p>
          <textarea
            rows={3}
            placeholder="e.g. Completed morning rounds, cleaned all seeker rooms, suggested new shift handover system..."
            className="w-full min-h-[100px] rounded-2xl resize-none p-4 bg-white/5 border border-white/10 text-sm text-white placeholder:text-slate-500 outline-none focus:border-orange-500/50 transition-all"
          ...
          <button
            ...
            className="w-full py-3 mt-3 rounded-2xl bg-orange-500 text-white text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all disabled:opacity-50"
          >
