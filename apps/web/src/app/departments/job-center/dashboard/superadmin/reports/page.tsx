      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Comprehensive reports including seeker summaries and staff payroll</p>
          </div>
          {generated && (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <h2 className="font-bold text-gray-800 flex items-center gap-2"><Calendar className="w-5 h-5 text-orange-500" /> Select Period</h2>
            {/* View Toggle */}
              ...
              {(['daily', 'weekly', 'monthly'] as const).map(t => (
                <button
                  ...
                  className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${
                    reportType === t ? 'bg-white shadow-sm text-orange-600 font-bold' : 'text-gray-400 hover:text-gray-755'
                  }`}
                >
                  {t}
                </button>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-end">
            {reportType === 'daily' && (
              <div className="flex-1 w-full">
                ...
                <input
                  ...
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-orange-500 text-black font-bold"
                />
              </div>
            )}
            ...
            <button
              ...
              disabled={generating}
              className="w-full sm:w-auto bg-orange-600 hover:bg-orange-700 text-white px-8 py-2.5 rounded-xl font-medium text-sm transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
            >
        ...
        {generated && reportData && (
          <div id="JOBCENTER-report-print" ref={printRef} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-10 space-y-10">

            {/* Report Header */}
            <div className="text-center border-b border-gray-200 pb-6">
              <h2 className="text-2xl font-black text-gray-900">Khan Hub Job Center — Super Admin Report</h2>
              <p className="text-lg font-bold text-orange-700 mt-1">{reportData.reportLabel}</p>
            ...
            {reportData.staffSalaries.length > 0 && (
              <div>
                <h3 className="text-lg font-bold text-gray-800 mb-3 flex items-center gap-2"><UserCog className="w-5 h-5 text-orange-500" /> Staff Payroll Summary ({reportData.reportLabel.split('—')[1]?.trim() || reportData.reportLabel})</h3>
                <div className="overflow-x-auto rounded-xl border border-gray-200">
