...
export default function ProfilePage() {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const url = await uploadToCloudinary(file, 'Khan Hub/job-center/profile');
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        ...
        const fallbackSnap = await getDocs(query(collection(db, 'jobcenter_staff'), where('loginUserId', '==', parsed.uid)));
        let sData = null;
        ...
        const primaryDept = merged.department || 'jobcenter';
  ...
  if (loading || visibilityLoading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#f8f9fa]">
      <Loader2 className="animate-spin text-amber-600 w-10 h-10 mb-4" />
      <p className="text-xs font-medium text-gray-400 tracking-widest uppercase">Synchronizing Data Nexus...</p>
    </div>
  );

  return (
    <div className="min-h-screen bg-[#F9FAFB] text-gray-900 font-sans selection:bg-amber-100">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
             ...
             <button 
               ...
               className="p-2.5 text-gray-400 hover:text-amber-600 hover:bg-amber-50 rounded-xl transition-all tooltip" 
               title="Sign Out"
             >
               <LogOut size={20} />
             </button>
        ...
        <aside className="lg:col-span-4 space-y-6">
          <div className={`${cardStyle} p-8 text-center relative overflow-hidden`}>
            ...
            {sections.designation !== false && (
              <p className="text-amber-600 font-semibold text-xs uppercase tracking-wider mt-1 bg-amber-50 px-3 py-1 rounded-full inline-block">
            ...
            <div className="mt-8 space-y-2">
              <div className="flex items-center gap-3 p-3.5 bg-gray-50/50 rounded-xl border border-gray-50 text-left group hover:border-gray-200 transition-colors">
                <Mail size={16} className="text-gray-400 group-hover:text-amber-500 transition-colors" />
                <span className="text-sm text-gray-600 font-medium truncate">{profile?.email}</span>
              </div>
              <div className="flex items-center gap-3 p-3.5 bg-gray-50/50 rounded-xl border border-gray-50 text-left group hover:border-gray-200 transition-colors">
                <Phone size={16} className="text-gray-400 group-hover:text-amber-500 transition-colors" />
                ...
                {sections.attendance !== false && (
                  <div>
                    ...
                    <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full bg-amber-500 transition-all duration-700" style={{ width: `${attendancePerformance}%` }}></div>
                    </div>
                  </div>
                )}
                    ...
                    return (
                      <div key={task.id} className="group bg-white border border-gray-100 rounded-xl p-5 flex flex-col md:flex-row md:items-center gap-4 hover:border-gray-300 transition-all shadow-sm hover:shadow-md">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {task.points && (
                            <span className="text-[11px] font-black text-amber-600 bg-amber-50 px-3 py-1.5 rounded-md">
                              +{task.points} PTS
                            </span>
                          )}
                        </div>
              ...
              <div ref={el => { sectionRefs.current['score'] = el; }} id="section-score" className={`${cardStyle} p-6 scroll-mt-[140px]`}>
                 <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 pb-4 border-b border-gray-50">
                  ...
                  <input
                    ...
                    className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 text-sm font-bold text-gray-700 outline-none focus:ring-2 focus:ring-amber-500/20 w-full sm:w-auto"
                  />
                </div>
            ...
            {/* --- IDENTITY SECTION --- */}
            {
              <div ref={el => { sectionRefs.current['profile'] = el; }} id="section-profile" className={`${cardStyle} p-6 scroll-mt-[140px]`}>
                ...
                <div className="mt-8 bg-gray-50 border border-gray-100 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-4">
                   <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-white rounded-xl border border-gray-100 flex items-center justify-center text-amber-600">
                        <Award size={24} />
                      </div>
                      <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200 text-gray-900">
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <div>
                <h3 className="text-sm font-black uppercase tracking-widest text-amber-500">Salary Breakdown</h3>
            ...
            <div className="p-8 overflow-y-auto space-y-6 flex-1 text-xs">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-amber-50/30 rounded-2xl border border-amber-100/50">
                  <p className="text-[9px] font-black text-amber-500 uppercase tracking-widest mb-1">Monthly Base Salary</p>
                  <p className="text-lg font-black text-gray-900">₨{Number(profile?.monthlySalary || profile?.salary || 0).toLocaleString()}</p>
                </div>
                <div className="p-4 bg-amber-50/30 rounded-2xl border border-amber-100/50">
                  <p className="text-[9px] font-black text-amber-500 uppercase tracking-widest mb-1">Net Till Date Earned</p>
                  <p className="text-lg font-black text-amber-600">₨{salaryDetails.estimatedSalary.toLocaleString()}</p>
                </div>
              </div>
