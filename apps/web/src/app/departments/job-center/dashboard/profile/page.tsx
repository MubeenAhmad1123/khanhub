...
export default function ProfilePage() {
  ...
  const fetchMetrics = useCallback(async (sId: string) => {
    try {
      const [attSnap, dutySnap, dressSnap, pointsSnap, taskSnap, fineSnap] = await Promise.all([
        getDocs(query(collection(db, `jobcenter_attendance`), where('staffId', '==', sId), orderBy('date', 'desc'), limit(30))),
        getDocs(query(collection(db, `jobcenter_duty_logs`), where('staffId', '==', sId), orderBy('date', 'desc'), limit(30))),
        getDocs(query(collection(db, `jobcenter_dress_logs`), where('staffId', '==', sId), orderBy('date', 'desc'), limit(30))),
        getDocs(query(collection(db, `jobcenter_growth_points`), where('staffId', '==', sId), limit(1))),
        getDocs(query(collection(db, `jobcenter_special_tasks`), where('staffId', '==', sId), orderBy('date', 'desc'), limit(30))),
        getDocs(query(collection(db, `jobcenter_fines`), where('staffId', '==', sId), orderBy('date', 'desc'), limit(30)))
      ]);
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        setLoading(true);
        const userSnap = await getDoc(doc(db, 'jobcenter_users', parsed.uid));
        if (!userSnap.exists()) { router.push('/departments/job-center/login'); return; }
        ...
        if (parsed.role === 'seeker' && uData.seekerId) {
          ...
          let sSnap = await getDocs(query(collection(db, 'jobcenter_staff'), where('loginUserId', '==', parsed.uid)));
  ...
  const handleUploadPhoto = async (file: File) => {
    try {
      ...
      await updateDoc(doc(db, 'jobcenter_users', session.uid), { photoUrl: url });
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-[#f0f2f5]">
      <div className="animate-spin text-orange-600"><Loader2 size={40} /></div>
    </div>
  );

  return (
    <div className="min-h-screen bg-[#f0f2f5] pb-24 text-slate-800 font-sans selection:bg-orange-100">
      <div className="sticky top-0 z-50 px-4 py-6 md:px-12 bg-[#f0f2f5]/80 backdrop-blur-md border-b border-white/20">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-2xl ${neumorphicOutset} bg-[#f0f2f5] text-orange-600`}>
              <Briefcase size={28} className="drop-shadow-sm" />
            </div>
            <div>
              <h1 className="text-2xl md:text-3xl font-black tracking-tight text-slate-900">Job Center Portal</h1>
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className={`p-8 rounded-[3rem] ${glassStyle} flex flex-col items-center relative overflow-hidden`}>
            ...
            <div className="relative group">
              ...
              <button onClick={() => fileRef.current?.click()} className={`absolute -bottom-2 -right-2 p-3 rounded-2xl bg-orange-600 text-white shadow-xl hover:scale-110 transition-transform`}><Camera size={18} /></button>
            ...
            <p className="text-orange-600 font-bold text-sm tracking-wide uppercase mt-1">{profile?.designation || (session?.role === 'seeker' ? 'Job Seeker' : 'Staff Member')}</p>
        ...
        <div className="lg:col-span-8 space-y-8">
          {session?.role === 'seeker' ? (
             <div className={`p-8 md:p-10 rounded-[3.5rem] ${glassStyle}`}>
                <h3 className="text-2xl font-black text-slate-900 mb-8 flex items-center gap-3"><Heart className="text-rose-500" /> Linked Seeker Details</h3>
                {seekerDoc ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#f0f2f5]`}>
                      <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Seeker Name</p>
                      <p className="text-lg font-black text-slate-900">{seekerDoc.name}</p>
                    </div>
                    <div className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#f0f2f5]`}>
                      <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Admission Date</p>
                      <p className="text-lg font-black text-slate-900">{formatDateDMY(seekerDoc.admissionDate)}</p>
                    </div>
                    <div className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#f0f2f5]`}>
                      <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Status</p>
                      <span className={`px-4 py-1 rounded-xl text-[10px] font-black uppercase ${seekerDoc.isActive ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                        {seekerDoc.isActive ? 'Active' : 'Inactive'}
                      </span>
                    </div>
                  </div>
                ) : (
                  <p className="text-slate-400 italic">No seeker profile linked to this user.</p>
                )}
             </div>
          ) : (
            <>
              <div className={`p-2 rounded-[2rem] ${neumorphicOutset} bg-[#f0f2f5] flex overflow-x-auto no-scrollbar gap-1`}>
                {[
                  ...
                  <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex items-center gap-2 px-6 py-4 rounded-[1.5rem] text-sm font-black transition-all whitespace-nowrap ${activeTab === tab.id ? `bg-orange-600 text-white shadow-[4px_4px_10px_rgba(249,115,22,0.3)] scale-105` : `text-slate-400 hover:text-slate-600 hover:bg-slate-200/50`}`}>
                    {tab.icon} {tab.label}
                  </button>
                ))}
              </div>
                    ...
                    <div className="space-y-4">
                      {specialTasks.map(task => (
                        <div key={task.id} className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#f0f2f5] flex items-center justify-between group hover:scale-[1.01] transition-transform`}>
                          ...
                          <div className="px-3 py-1 rounded-xl bg-white border border-slate-100 text-[10px] font-black text-orange-600">+{task.points} PTS</div>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {attendance.map(log => (
                        <div key={log.id} className={`p-5 rounded-[2rem] ${neumorphicOutset} bg-[#f0f2f5] flex items-center justify-between`}>
                          <div><p className="font-black text-slate-900">{formatDateDMY(log.date)}</p><p className="text-[10px] font-black text-slate-400 uppercase">{log.arrivalTime || '--:--'} to {log.departureTime || '--:--'}</p></div>
                          <div className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase ${log.status === 'present' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-700'}`}>{log.status}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {activeTab === 'finance' && (
                  <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                      <div className={`p-8 rounded-[3rem] ${neumorphicOutset} bg-orange-600 text-white`}><p className="text-xs font-black uppercase opacity-60">Estimated Payable</p><h4 className="text-4xl font-black mt-2">Rs. {totalEarnings.toLocaleString()}</h4></div>
                    ...
                    <div className="space-y-4">
                      {fines.map(fine => (
                        <div key={fine.id} className={`p-6 rounded-3xl ${neumorphicInset} flex items-center justify-between`}>
                           <div className="flex items-center gap-4"><div className="p-3 rounded-2xl bg-rose-50 text-rose-500"><AlertCircle size={20} /></div><div><p className="font-black text-slate-900">{fine.reason}</p><p className="text-[10px] font-black text-slate-400">{formatDateDMY(fine.date)}</p></div></div>
                     ...
                     <div className="space-y-4">
                        {duties.map(duty => (
                          <div key={duty.id} className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#f0f2f5] flex items-center justify-between`}>
                            ...
                            <div className="px-3 py-1 rounded-xl bg-white border border-slate-100 text-[10px] font-black text-orange-600">+{duty.points} PTS</div>
                          </div>
                        ))}
                     </div>
                  </div>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {dressLogs.map(log => (
                          <div key={log.id} className={`p-6 rounded-3xl ${neumorphicOutset} bg-[#f0f2f5] flex items-center justify-between`}>
                            <div className="flex items-center gap-4"><div className={`p-3 rounded-2xl ${log.status === 'yes' ? 'bg-orange-100 text-orange-600' : 'bg-slate-100 text-slate-400'}`}><Shirt size={20} /></div><div><p className="font-black text-slate-900">{formatDateDMY(log.date)}</p><p className="text-[10px] font-black text-slate-400">{log.status === 'yes' ? 'Compliant' : 'Non-Compliant'}</p></div></div>
                            {log.status === 'yes' && <div className="px-3 py-1 rounded-xl bg-white border border-slate-100 text-[10px] font-black text-orange-600">+{log.points} PTS</div>}
                          </div>
                        ))}
                     </div>
                  </div>
                  ...
                  <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       <div className="space-y-6">
                          <div><p className="text-[10px] font-black uppercase text-slate-400 mb-2 px-2">National ID (CNIC)</p><div className={`p-5 rounded-[2rem] ${neumorphicInset} bg-white flex items-center gap-4`}><Shield className="text-orange-500" size={20} /><p className="text-sm font-bold text-slate-900">{profile?.cnic || 'Not provided'}</p></div></div>
                          <div><p className="text-[10px] font-black uppercase text-slate-400 mb-2 px-2">Home Address</p><div className={`p-5 rounded-[2rem] ${neumorphicInset} bg-white flex items-start gap-4`}><MapPin className="text-orange-500 mt-1" size={20} /><p className="text-sm font-bold text-slate-900 leading-relaxed">{profile?.address || 'No address registered.'}</p></div></div>
                       </div>
                       <div className="space-y-6">
                          <div><p className="text-[10px] font-black uppercase text-slate-400 mb-2 px-2">Work Schedule</p><div className={`p-5 rounded-[2rem] ${neumorphicInset} bg-white flex items-center gap-4`}><Clock className="text-orange-500" size={20} /><p className="text-sm font-bold text-slate-900">{profile?.dutyStartTime || '09:00'} - {profile?.dutyEndTime || '17:00'}</p></div></div>
                       </div>
                    </div>
                  </div>
                )}
