...
export default function DayClosePage() {
  ...
  const handleCloseDay = async () => {
    if (!actualCash) return alert('Enter cash in hand');
    ...
    return (
      <div className="min-h-screen bg-[#FCFBF8] flex items-center justify-center">
        <div className="text-center animate-pulse">
           ...
           <p className="text-zinc-400 font-black uppercase tracking-[0.2em] text-[10px]">Loading...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#FCFBF8] pb-20 font-sans selection:bg-indigo-100 selection:text-indigo-900">
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-zinc-100 px-4 py-4 md:px-8">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-4">
           <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-2xl font-black text-zinc-900 tracking-tight leading-tight">End of Day</h1>
              <div className="flex items-center gap-2 mt-0.5">
                <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse" />
                <p className="text-[11px] font-black text-zinc-400 uppercase tracking-widest">Verify today's cash before closing</p>
              </div>
            </div>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 md:py-10 md:px-8">
        {isDayClosed ? (
          <div className="bg-white p-6 sm:p-12 md:p-16 rounded-[2rem] sm:rounded-[3rem] border border-zinc-100 shadow-2xl shadow-indigo-100/30 text-center animate-in zoom-in-95 duration-700">
             ...
             <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-zinc-900 tracking-tight mb-4">Day Closed</h2>
             <p className="text-zinc-500 font-bold max-w-sm mx-auto leading-relaxed mb-12 text-sm sm:text-base">
                Today's transactions are finalized. No further changes can be made.
             </p>
             <button 
                onClick={() => router.push('/hq/dashboard/cashier/daily-report')}
                className="px-6 py-4 sm:px-10 sm:py-5 bg-indigo-600 text-white rounded-[1.5rem] sm:rounded-[2rem] font-black text-sm uppercase tracking-[0.2em] hover:bg-zinc-900 transition-all active:scale-95 shadow-2xl shadow-indigo-600/20"
             >
                View Report
             </button>
          </div>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
             <div className="bg-white p-4 sm:p-8 md:p-12 rounded-[2rem] sm:rounded-[3rem] border border-zinc-100 shadow-xl shadow-zinc-200/40 relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-12 opacity-[0.03] transition-transform group-hover:rotate-12 duration-1000 pointer-events-none">
                ...
                <div className="relative z-10">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
                    <div className="flex items-center gap-4">
                       <div className="w-2 h-8 sm:h-10 bg-indigo-600 rounded-full" />
                       <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-zinc-900 tracking-tight">Verify Cash</h2>
                    </div>
                    <div className="flex items-center gap-2 px-4 py-2 bg-zinc-50 rounded-xl border border-zinc-100 self-start sm:self-auto">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                     <div className="p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] bg-zinc-50/50 border border-zinc-100 flex flex-col justify-between group hover:bg-white hover:shadow-xl hover:shadow-zinc-200/50 transition-all duration-500 border-dashed">
                        <div>
                          <p className="text-[10px] font-black text-zinc-400 tracking-widest uppercase mb-2">Opening Cash</p>
                          <p className="text-xl sm:text-2xl font-black text-zinc-900 tracking-tight">PKR {openingBalance.toLocaleString()}</p>
                        </div>
                     ...
                     <div className="p-6 sm:p-10 rounded-[2rem] sm:rounded-[3rem] bg-indigo-600 text-white shadow-2xl shadow-indigo-600/20 flex flex-col items-center justify-center md:scale-105 transition-transform hover:scale-[1.08] duration-500">
                        <p className="text-[10px] font-black text-indigo-200 tracking-widest uppercase mb-2">Expected Cash</p>
                        <p className="text-3xl sm:text-4xl font-black text-white tracking-tighter">PKR {expectedClosing.toLocaleString()}</p>
                     </div>
                  </div>
                    ...
                    <div className="space-y-6 sm:space-y-10">
                       <div className="space-y-4 sm:space-y-6">
                          ...
                          <div className="relative group">
                             <div className="absolute left-6 sm:left-8 top-1/2 -translate-y-1/2 text-xl sm:text-3xl font-black text-zinc-200 group-focus-within:text-indigo-600 transition-colors duration-500">PKR</div>
                             <input 
                               type="number"
                               value={actualCash}
                               onChange={(e) => setActualCash(e.target.value)}
                               placeholder="00"
                               className="w-full pl-20 sm:pl-24 pr-6 sm:pr-10 py-6 sm:py-10 bg-zinc-50 border-2 border-zinc-100 rounded-[1.5rem] sm:rounded-[2.5rem] font-black text-3xl sm:text-5xl text-zinc-900 outline-none focus:bg-white focus:border-indigo-600 transition-all duration-500 placeholder:text-zinc-100 shadow-inner focus:shadow-2xl focus:shadow-indigo-100/50"
                             />
                          </div>
                       </div>
                       ...
                       {actualCash && (
                        <div className={cn(
                          "p-6 sm:p-10 rounded-[1.5rem] sm:rounded-[2.5rem] flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-in slide-in-from-top-4 duration-500 shadow-xl",
                          ...
                          variance > 0 ? "bg-indigo-600 text-white shadow-indigo-200/50" : "bg-rose-500 text-white shadow-rose-200/50"
                           ...
                           <div className="flex items-center gap-4 sm:gap-6">
                              <div className="p-4 sm:p-5 rounded-2xl shadow-inner bg-white/20">
                                 {variance === 0 ? <CheckCircle2 className="w-6 h-6 sm:w-8 sm:h-8" /> : <AlertCircle className="w-6 h-6 sm:w-8 sm:h-8" />}
                              </div>
                              <div>
                                 <p className="text-[10px] font-black uppercase tracking-widest opacity-70">Difference</p>
                                 <p className="text-xl sm:text-2xl font-black tracking-tight">PKR {Math.abs(variance).toLocaleString()}</p>
                              </div>
                           </div>
                    ...
                    <div className="space-y-6 sm:space-y-8 h-full flex flex-col">
                       <div className="flex-grow space-y-4 sm:space-y-6">
                          ...
                          <textarea 
                            ...
                            placeholder="Enter any details or reasons for differences here..."
                            className="w-full p-5 sm:p-8 bg-zinc-50 border-2 border-zinc-100 rounded-[1.5rem] sm:rounded-[2.5rem] font-bold text-base text-zinc-900 outline-none focus:bg-white focus:border-indigo-600 transition-all duration-500 placeholder:text-zinc-200 min-h-[160px] sm:min-h-[220px]"
                          />
                       </div>
                       ...
                       <button 
                         ...
                         disabled={!actualCash || isSubmitting}
                          className="w-full py-6 sm:py-8 md:py-10 bg-indigo-600 hover:bg-zinc-900 text-white rounded-[1.5rem] sm:rounded-[2.5rem] font-black text-base sm:text-xl uppercase tracking-[0.2em] flex items-center justify-center gap-4 sm:gap-5 transition-all shadow-2xl shadow-indigo-600/30 active:scale-95 disabled:opacity-50 disabled:grayscale group"
                       >
                         {isSubmitting ? <Loader2 className="w-6 h-6 sm:w-8 sm:h-8 animate-spin" /> : (
                           <>
                             <FileCheck className="w-6 h-6 sm:w-8 sm:h-8 transition-transform group-hover:-translate-y-1" />
                             End Day
                           </>
                         )}
                       </button>
