              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.map((t, index) => (
                    <tr key={t.id} style={{ animationDelay: `${index * 30}ms` }} className="animate-in fade-in duration-300 border-b border-white/5 hover:bg-white/5 transition-colors duration-150">
                      <td className="px-4 py-3.5 text-xs text-gray-500 whitespace-nowrap">{t.date ? formatDateDMY(t.date) : '—'}</td>
                      ...
                      <td className="px-4 py-3.5 text-xs text-gray-500 whitespace-nowrap">{t.patientName || '—'}</td>
                ...
                <tfoot>
                  <tr className="bg-amber-500/5 border-t border-amber-500/10">
                    ...
                    <td className="px-4 py-3 text-xs font-black text-green-400">+₨{totalIncome.toLocaleString()}</td>
                    <td className="px-4 py-3 text-xs font-black text-red-400">-₨{totalExpense.toLocaleString()}</td>
                    <td className="px-4 py-3 text-xs font-black text-white">₨{(totalIncome - totalExpense).toLocaleString()}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
