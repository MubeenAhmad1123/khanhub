...
function RejectModal({
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-md mx-auto p-6 shadow-2xl z-10 max-h-[95vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-black text-gray-900">Reject this transaction?</h2>
          <button type="button" onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100">
            <X className="w-4 h-4 text-gray-500" />
          </button>
        </div>
        <div className="mb-5 p-4 rounded-2xl bg-red-50 border border-red-100">
          <div className="text-sm font-bold text-red-800">{entityName(tx)}</div>
          <div className="text-2xl font-extrabold text-red-900 mt-1">{fmtPKR(tx.amount)}</div>
        </div>
        <div className="mb-3">
          <div className="text-xs font-black uppercase tracking-wider text-gray-400 mb-2">Preset reason</div>
          <div className="flex flex-wrap gap-2">
            {REJECT_PRESETS.map((p) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-bold border transition ${
                  reason === p ? 'bg-red-600 text-white border-red-600' : 'bg-white text-gray-700 border-gray-200 hover:border-red-300'
                }`}
              >
                {p}
              </button>
        ...
        <textarea
          ...
          className="w-full rounded-2xl border border-gray-200 p-3 text-sm text-gray-800 outline-none focus:border-red-400 resize-none"
        />
        <button
          type="button"
...
function BulkConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <div className="relative bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-lg mx-auto p-6 shadow-2xl z-10 max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-black text-gray-900">Approve {selected.length} transactions?</h2>
        ...
        <ul className="space-y-2 mb-4 max-h-[40vh] overflow-y-auto">
          {items.map((tx) => (
            <li key={tx.id} className="flex items-center justify-between py-2 border-b border-gray-100 gap-3">
              <div className="min-w-0">
                <div className="text-sm font-bold text-gray-800 truncate">{entityName(tx)}</div>
                <div className="text-xs text-gray-400 truncate">{typeLabel(tx)}</div>
              </div>
              <div className="text-sm font-extrabold text-gray-900 shrink-0">{fmtPKR(tx.amount)}</div>
            </li>
          ))}
        </ul>
        <div className="p-3 rounded-2xl bg-green-50 border border-green-100 mb-5 flex items-center justify-between gap-3">
...
function TxCard({
  ...
  const hist = useEntityHistory(showHistory, tx.dept, eid, tx.id);
  ...
  return (
    <div
      className={`relative rounded-3xl border bg-white shadow-md transition-all duration-300 overflow-hidden max-w-[680px] mx-auto w-full ${
        phase === 'success' ? 'border-green-500 ring-2 ring-green-400/40' : phase === 'fail' ? 'border-red-500 ring-2 ring-red-400/40' : 'border-gray-100'
    ...
    >
      {phase === 'success' ? (
        <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center bg-white/40">
          <div className="tx-approve-overlay w-16 h-16 rounded-full bg-green-500 text-white flex items-center justify-center shadow-xl">
      ...
      {phase === 'fail' ? (
        <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center bg-white/40">
          <div className="tx-approve-overlay w-16 h-16 rounded-full bg-red-500 text-white flex items-center justify-center shadow-xl">
      ...
      <div className={`p-4 sm:p-6 ${showSelect ? 'pl-10 sm:pl-12' : ''}`}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="text-right shrink-0">
            <div className="text-2xl font-extrabold text-gray-900">{fmtPKR(tx.amount)}</div>
        ...
        <div className="mt-4 space-y-4">
          <section className="rounded-2xl bg-[color:var(--filter-bg,#f8fafc)] border border-gray-100 p-4 space-y-2">
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Entity info</p>
            <div className="flex gap-2 text-sm">
              <span aria-hidden>👤</span>
              <div>
                <p className="text-[10px] text-gray-500 uppercase tracking-wide">Student / Patient</p>
            ...
            {tx.dept === 'spims' && (enrich?.course || enrich?.session) ? (
              <div className="flex gap-2 text-sm text-gray-800">
                <span aria-hidden>🎓</span>
                <div>
                  <p className="text-[10px] text-gray-500 uppercase tracking-wide">Course · Session</p>
            ...
            {phref ? (
              <Link href={phref} className="inline-flex text-sm font-bold text-blue-600 hover:underline">
                View Full Profile →
              </Link>
            ) : null}
          </section>

          <section className="rounded-2xl border border-gray-100 p-4 space-y-2 bg-white">
            ...
            <div className="grid sm:grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <p className="font-medium text-gray-800">{fmtPKR(tx.amount)}</p>
              </div>
              <div>
                <p className="text-[10px] text-gray-500 uppercase tracking-wide">Payment date</p>
                <p className="font-medium text-gray-800">{fmtDate(tx.createdAt || tx.date || tx.transactionDate)}</p>
              </div>
              <div>
                <p className="text-[10px] text-gray-500 uppercase tracking-wide">Type</p>
                <p className="font-medium text-gray-800">{typ}</p>
              </div>
              {tx.description ? (
                <div className="sm:col-span-2">
                  <p className="text-[10px] text-gray-500 uppercase tracking-wide">Note</p>
                  <p className="text-sm font-medium text-gray-700 break-words">&quot;{tx.description}&quot;</p>
                </div>
              ) : null}
          ...
          <section className="rounded-2xl bg-gray-50 border border-gray-100 p-4 space-y-2">
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Submitted by</p>
            <div className="grid sm:grid-cols-2 gap-3 text-sm">
              <div>
                <p className="text-[10px] text-gray-500 uppercase tracking-wide">Cashier</p>
                <p className="font-medium text-gray-800">{tx.cashierName || tx.cashierId || '—'}</p>
              </div>
              <div>
                <p className="text-[10px] text-gray-500 uppercase tracking-wide">Cashier role</p>
                <p className="font-medium text-gray-800">{tx.cashierRole || '—'}</p>
              </div>
              <div className="sm:col-span-2">
                <p className="text-[10px] text-gray-500 uppercase tracking-wide">Submitted at</p>
                <p className="font-medium text-gray-800">{fmtTime(tx.createdAt || tx.date)}</p>
              </div>
              <div className="sm:col-span-2">
                <p className="text-[10px] text-gray-500 uppercase tracking-wide">Forwarded from</p>
                <p className="font-medium text-gray-800">{tx.forwardedFromLabel || tx.departmentName || 'SPIMS Portal'}</p>
              </div>
              <div className="sm:col-span-2">
                <button
                  type="button"
                  onClick={copyId}
                  className="text-left text-[10px] text-gray-400 hover:text-gray-600 font-mono break-all w-full"
                >
          ...
          <div>
            {tx.proofUrl ? (
              <button
                ...
                className="w-full rounded-2xl border border-green-200 bg-green-50 p-3 flex items-start gap-3 hover:bg-green-100 transition text-left"
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          <div className="border border-gray-100 rounded-2xl overflow-hidden">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 text-xs font-black text-gray-600 uppercase tracking-widest hover:bg-gray-50"
            >
              <span>Previous payments for {name}</span>
            ...
            {showHistory ? (
              <div className="px-2 pb-2 overflow-x-auto">
                    ...
                    <tbody>
                      {hist.rows.map((r) => (
                        <tr key={r.id} className="border-t border-gray-100">
                          <td className="px-2 py-2 whitespace-nowrap">{fmtDate(r.createdAt)}</td>
                          <td className="px-2 py-2">{fmtPKR(r.amount)}</td>
                          <td className="px-2 py-2">{typeLabel(r)}</td>
                ...
                {phref ? (
                  <Link href={phref} className="block text-center py-2 text-xs font-bold text-blue-600">
                    View All →
                  </Link>
                ) : null}
              </div>
...
export default function HqApprovalsPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 overflow-x-hidden w-full max-w-full pb-36 text-gray-900">
      <DesignTokensStyle />
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-5 py-6 space-y-5">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-black tracking-tight text-gray-900 flex flex-wrap items-center gap-2">
              Approvals
              {pendingCount > 0 ? (
            ...
            <p className="mt-1 text-sm text-gray-500">Review incoming payments with confidence.</p>
          </div>
        </div>
          ...
          {(
            ...
            <button
              ...
              className={`py-3 rounded-2xl text-[11px] sm:text-xs font-black uppercase tracking-widest transition border ${
                tab === t.id ? 'bg-gray-900 text-white border-gray-900 shadow-md' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300'
              }`}
            >
              {t.label}
            </button>
        ...
        {selectedEntity ? (
          <div className="space-y-4">
            ...
            <div className="rounded-3xl border border-gray-100 bg-white shadow-md p-5 max-w-[680px] mx-auto w-full">
              <p className="text-xs font-black uppercase tracking-widest text-gray-400">Entity</p>
              ...
              {entitySummary?.totalPackage != null ? (
                ...
                <p className="text-sm text-gray-500 mt-2">Loading package info…</p>
              )}
            </div>
              ...
              <div className="rounded-3xl bg-white dark:bg-gray-800 shadow-md border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div className={`${filtersPanelOpen ? 'block' : 'hidden'} lg:block p-4 sm:p-5 space-y-4`}>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Amount range</p>
                      <select
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div ref={searchWrapRef} className="relative">
                      <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Entity search</p>
                    ...
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Proof</p>
                      <PillGroup
                  ...
                  <div className="lg:grid lg:grid-cols-2 lg:gap-4 space-y-3 lg:space-y-0">
                    ...
                    <div className={`${moreFiltersOpen ? 'block' : 'hidden'} lg:grid lg:grid-cols-2 lg:col-span-2 lg:gap-4 lg:!block space-y-4`}>
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Cashier name</p>
                        <select
                  ...
                  {hasAnyFilter ? (
                    <button
                      ...
                      className="text-sm font-black text-red-600 hover:text-red-800 underline underline-offset-2"
                    >
                      Clear All Filters
                    </button>
                  ) : null}
            ...
            {!loading && !error && filteredRows.length > visibleRows.length ? (
              <div className="flex justify-center pt-2">
                <button
                  ...
                  className="h-12 px-8 rounded-2xl border border-gray-300 bg-white text-sm font-black text-gray-800 hover:bg-gray-50"
                >
                  Load More
                </button>
              </div>
      ...
      {selected.size > 0 && isPendingTab ? (
        <div className="fixed bottom-0 left-0 right-0 z-[100] bg-white border-t border-gray-200 shadow-[0_-8px_30px_rgba(0,0,0,0.08)] px-4 py-4 pb-6">
          <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <div className="flex gap-2 justify-center">
              <button
                ...
                className="h-12 px-5 rounded-2xl border border-gray-200 text-sm font-black text-gray-700 hover:bg-gray-50"
              >
                Clear Selection
              </button>
              <button
...
function EmptyApprovals({ tab }: { tab: ApprovalsTab }) {
  ...
  return (
    <div className="rounded-3xl border border-green-100 bg-white p-10 text-center shadow-sm max-w-[680px] mx-auto">
      <div className="text-5xl mb-4" aria-hidden>
        {isPending ? '✅' : '📋'}
      </div>
      <div className="text-lg font-black text-gray-900">{isPending ? 'No pending approvals' : 'No transactions found'}</div>
