...
export type SukoonRole = 'admin' | 'staff' | 'family' | 'cashier' | 'superadmin';

// ─── SUKOON USER (Auth) ───────────────────────────────────────────────────────

export interface SukoonUser {
  uid: string;
  customId: string;
  name: string;
  displayName?: string;
...
// ─── CLIENT (Full Admission Form Data) ─────────────────────────────────────

export interface Client {
  id: string;

  // Basic Identity
  inpatientNumber: string;         // e.g. "SUKOON-058"
...
// One document per client per date
// Collection: sukoon_daily_activities
...
// Collection: sukoon_therapy_sessions
...
// Collection: sukoon_medication_records
...
// Collection: sukoon_weekly_progress
...
export interface StaffMember {
  ...
  name: string;
  fatherName: string;
  employeeId: string;           // e.g. "SUKOON-STF-001"
  designation: string;          // e.g. "Counselor", "Security Guard", "Nurse"
  department: 'sukoon';          // for now always sukoon
  gender: StaffGender;
  phone?: string;
  photoUrl?: string;
  ...
  loginUserId?: string;         // uid in sukoon_users for portal login
  role: SukoonRole;              // used for UI role indicators
...
// Collection: sukoon_growth_points
