...
export type HospitalRole = 'admin' | 'staff' | 'family' | 'cashier' | 'superadmin';
...
export interface Patient {
  ...
  // Basic Identity
  inpatientNumber: string;         // e.g. "HOSPITAL-058"
...
// Collection: hospital_daily_activities
...
// Collection: hospital_therapy_sessions
...
// Collection: hospital_medication_records
...
// Collection: hospital_weekly_progress
...
export interface StaffMember {
  ...
  name: string;
  fatherName: string;
  employeeId: string;           // e.g. "HOSPITAL-STF-001"
  designation: string;          // e.g. "Counselor", "Security Guard", "Nurse"
  department: 'hospital';          // for now always hospital
  gender: StaffGender;
  phone?: string;
  photoUrl?: string;
  ...
  loginUserId?: string;         // uid in hospital_users for portal login
  role: HospitalRole;              // used for UI role indicators
...
// One doc per staff per date
// Collection: hospital_dress_logs
...
// Collection: hospital_growth_points
